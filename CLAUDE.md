...
#### QA Node Reporting Guidelines

When completing QA validation, the `report_done()` call must include **actionable context** for future reference, not just a pass/fail verdict:

**Required sections in QA report:**

1. **PR Link** — Direct link to the PR for easy access

2. **Dev Server Setup** — Exact command to run:
   ```bash
   make dev PORT=8383 DB_PATH=/tmp/neokai-$(basename $PWD).db
   ```

3. **Key Browser Scenarios** — Specific user flows to verify:
   - Step-by-step actions and expected outcomes
   - Edge cases to test
   - What "pass" looks like for each scenario

4. **E2E Tests to Run** — Exact command:
   ```bash
   make run-e2e TEST=tests/features/your-feature.e2e.ts
   ```

5. **Baseline Test Results** — Already-passing tests as confidence baseline:
   - Typecheck, lint, unit tests
   - Run `make test-daemon` and `make test-web` for baseline

6. **Issues Found & Fixed** — Detailed breakdown:
   - What failed and why
   - What was changed to fix it
   - Commits with the fixes
...
**PR:** https://github.com/lsm/neokai/pull/1234

**Dev Server:** `make dev PORT=8383 DB_PATH=/tmp/neokai-$(basename $PWD).db`

**Browser Scenarios:**
1. Click "New Session" → session creates immediately (no workspace modal) ✅
2. WorkspaceSelector appears inline with history dropdown ✅
3. Select workspace from history → worktree/direct toggle appears ✅
4. Click "Skip" → selector dismisses, chat remains usable ✅

**E2E Test:** `make run-e2e TEST=tests/features/workspace-selector.e2e.ts` ✅

**Baseline Tests:**
- Typecheck ✅, Lint ✅, Unit Tests (75) ✅

**Issues Fixed:**
- Fixed WorkspaceSelector z-index (chat footer blocking clicks) — commit abc123
- Fixed NewSessionModal tests (removed stale workspace input refs) — commit def456

**Committed:** abc123, def456
```
