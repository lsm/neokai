...
2. Replace the single emoji model button with a compact text display: `[icon] [model-name-short]` where `model-name-short` is produced by this exact transform rule: split the full model ID string on `-`, discard the first segment if it equals `claude` or `gpt` or another known provider prefix, then take the next segment and capitalize its first letter. Examples: `claude-opus-4-5` → segments `["claude","opus","4","5"]` → drop `"claude"` → take `"opus"` → `"Opus"`; `claude-sonnet-4-5` → `"Sonnet"`; `claude-haiku-3-5` → `"Haiku"`; `gpt-4o` → drop `"gpt"` → `"4o"`; `claude-3-5-sonnet-20241022` → take segment index 1 (`"3"`) would be wrong — for this legacy pattern take the last alphabetic-only segment → `"Sonnet"`. Implement a helper function `shortModelName(modelId: string): string` that encodes this logic; add it to `packages/web/src/lib/session-utils.ts`. Use `text-xs font-medium text-gray-300`.
