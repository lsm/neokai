...
1. In `session-lifecycle.ts`, split the fallback logic based on **session type** (not session ID prefix). At line 91, `sessionType` is available as `params.sessionType ?? 'worker'`. The room-scoped session types are: `'room_chat'`, `'planner'`, `'coder'`, `'leader'`, `'general'`. Guard using a set:
   ...
   **Session type coverage**: The full `sessionType` union at `session-lifecycle.ts:53-62` includes: `room_chat`, `planner`, `coder`, `leader`, `general`, `worker`, `lobby`, `spaces_global`, `space_chat`, `neo`. Of these:
   ...
   - **Note on `space_task_agent`**: The DB schema `CHECK` constraint includes `space_task_agent` as a valid session type, but it is NOT in the `SessionLifecycle` params type union (lines 53-63). Space task agent sessions bypass `SessionLifecycle` entirely, so they are not affected by this guard. If they are ever routed through `SessionLifecycle` in the future, they should be added to `ROOM_SCOPED_SESSION_TYPES`.
...
2. Update the `worktree.cleanup` RPC handler in `session-handlers.ts` (line 661-662) — this is the **only known public call site**. The handler currently extracts `workspacePath` from the request payload (optional) and passes it through. Apply the fallback **at the RPC handler layer**: when the caller omits `workspacePath`, use `deps.config.workspaceRoot` as the default. **Add an explicit guard**: if both `params.workspacePath` and `deps.config.workspaceRoot` are undefined (possible after Milestone 5), return an RPC error: `"workspacePath is required when daemon has no default workspace"`. This surfaces a clean error instead of passing `undefined` to the worktree manager. Example: `const path = params.workspacePath ?? deps.config.workspaceRoot; if (!path) throw new Error('workspacePath is required...');`.
