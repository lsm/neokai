...
**Subtasks**:
1. In `room-handlers.ts` `room.update` handler, after a successful `defaultPath` change, update the room chat session's `workspacePath` via `sessionManager.updateSession(roomChatSessionId, { workspacePath: newDefaultPath })`. Add this alongside the existing `defaultModel` sync logic. **Method clarification**: Call `sessionManager.updateSession()` (at `session-manager.ts:313`, signature `updateSession(sessionId: string, updates: Partial<Session>): Promise<void>`). This delegates to `SessionLifecycle.update()` (at `session-lifecycle.ts:395-409`) which calls `agentSession.updateMetadata(updates)` when the session is live in cache, updating both the DB row and the in-memory `AgentSession` metadata. `resolveSessionContext` in `reference-handlers.ts:328` calls `getSessionAsync` which re-fetches from DB on cache miss. Both paths are covered — **no additional cache invalidation is needed**. Document this in the PR description for reviewers.
