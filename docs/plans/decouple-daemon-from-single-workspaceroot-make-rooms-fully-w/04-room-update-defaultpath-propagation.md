...
**Subtasks**:
1. In `room-handlers.ts` `room.update` handler, after a successful `defaultPath` change, update the room chat session's `workspacePath` via `sessionManager.updateSession(roomChatSessionId, { workspacePath: newDefaultPath })`. Add this alongside the existing `defaultModel` sync logic. **Important**: `updateSession` writes to the DB and updates in-memory `Session` metadata, but `resolveSessionContext` in `reference-handlers.ts:340` reads `sessionData.workspacePath` from the in-memory session object. Verify that `updateSession` also updates the in-memory session cache. If the `SessionManager` uses a cache that is not refreshed by `updateSession`, either: (a) call `sessionManager.reloadSession(roomChatSessionId)` after the update, or (b) invalidate the session from the cache so the next read fetches from DB. Without this, `@file`/`@folder` reference resolution will use the stale old path until process restart.
