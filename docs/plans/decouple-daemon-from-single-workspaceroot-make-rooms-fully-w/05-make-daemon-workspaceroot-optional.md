...
5. Update `FileIndex` to accept `undefined` root -- when undefined, `init()` is a no-op and searches return empty results. Add a guard at the top of `init()`. **Note**: Verify whether `FileIndex` is constructed in `app.ts` and/or in `rpc-handlers/index.ts` — ensure both instantiation sites are updated to handle optional `workspaceRoot`.
