...
**Description:** Currently the `TaskRow` in `SpaceDashboard.tsx` shows "blocked" status but does not display the reason. The `SpaceTaskPane.tsx` also lacks blocked reason display. Add visible blocked reason text to both components. The blocked reason should come from the task's `result` field or a new notification event.

**Subtasks:**
1. Read `packages/shared/src/types/space.ts` for `SpaceTask` interface -- check if there is a `blockedReason` field or if the `result` field is used.
2. Read `packages/daemon/src/lib/space/runtime/session-notification-sink.ts` for how blocked events carry reason text.
3. Read `packages/web/src/components/space/SpaceDashboard.tsx` `TaskRow` component.
4. Read `packages/web/src/components/space/SpaceTaskPane.tsx` for task detail view.
5. If `SpaceTask` lacks a `blockedReason` field, add one to the type and persistence layer, or use the existing `result` field for blocked tasks.
6. Update `TaskRow` in `SpaceDashboard.tsx` to show blocked reason text below the status when task is blocked.
7. Update `SpaceTaskPane.tsx` to show a prominent blocked reason banner when task status is blocked.
8. Add Vitest tests for both components verifying blocked reason renders.
9. Run `cd packages/web && bunx vitest run src/components/space/__tests__/SpaceDashboard.test.tsx` and similar.
