...
- **Concurrent messages**: Neo uses a message queue — if a second message arrives while the first is being processed, it is queued and sent after the current turn completes.
- **Session ID**: Always the literal string `'neo:global'`. On `clearSession`, messages are deleted (`DELETE FROM sdk_messages WHERE session_id = 'neo:global'`) and the session row is reset. The session ID never changes — no alias/pointer mechanism needed. This keeps LiveQuery filters, repository lookups, and provisioning logic simple.
...
- **Session clear vs action log**: `clearSession` deletes messages from `sdk_messages` and resets the session row, but preserves the same `'neo:global'` session ID. The `neo_action_log` table is independent — action history is preserved across session clears and visible in the Activity Feed.
