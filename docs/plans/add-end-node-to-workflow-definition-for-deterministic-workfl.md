...
2. In `packages/daemon/src/storage/repositories/space-workflow-repository.ts`:
   ...
   - In `updateWorkflow()` (line ~234), add handling for `endNodeId`. Use `'endNodeId' in params` (property-existence check) rather than `params.endNodeId !== undefined` to correctly handle `null` (which clears the field). When `params.endNodeId` is `null`, set `end_node_id = NULL` in the SET clause. The existing `startNodeId` on `UpdateSpaceWorkflowParams` is also `string | null` and the repository uses `params.startNodeId !== undefined` (line ~252) which correctly handles both `string` (update) and `null` (clear) since both pass the `!== undefined` check. The same `!== undefined` pattern works for `endNodeId?: string | null` — use it for consistency with `startNodeId`. Add an inline code comment: `// undefined = not provided (no change), null = clear the field, string = set new value`.
...
5. **Critical: Orchestration task completion on auto-complete.** Currently `report_workflow_done` does three things: (a) transitions the run to `completed`, (b) marks the Task Agent's orchestration task as `completed`, and (c) emits `space.task.completed`. When the runtime auto-completes a run via CompletionDetector, it only does (a). Add post-completion logic in the runtime's run completion path, **before** the `return` statement after `transitionStatus(runId, 'completed')`. The orchestration task completion must be `await`ed (since `setTaskStatus` is async and `processRunTick` is already async) — do NOT fire-and-forget. The logic:
   ...
   - **Status transition guard:** Only attempt to complete the orchestration task if its current status is `in_progress`. The orchestration task may still be `pending` or `draft` if the Task Agent session was just spawned but hasn't started yet. `VALID_SPACE_TASK_TRANSITIONS` does not allow `pending → completed` or `draft → completed` — calling `setTaskStatus` on a non-`in_progress` task would throw. If the task is not `in_progress`, skip the completion silently (the task will be cleaned up by the existing `cleanupTerminalExecutors` path).
   ...
   - **`daemonHub` injection:** `daemonHub` is not currently in `SpaceRuntimeConfig`. Add `daemonHub` to `SpaceRuntimeConfig` interface. Then ensure `SpaceRuntimeService` propagates it: `SpaceRuntimeService` receives `daemonHub` in its own config (`SpaceRuntimeServiceConfig`, line ~71) and passes it to `new SpaceRuntime(config)` at line ~83 — add `daemonHub: this.config.daemonHub` to that config object. This follows the existing pattern of how `taskRepo`, `workflowRunRepo`, etc. are present in both configs.
6. Add unit tests in `packages/daemon/tests/unit/space/completion-detector.test.ts`:
   ...
   - Test: workflow with `endNodeId` completes for each status in `TERMINAL_TASK_STATUSES` (including `completed`, `needs_attention`, `cancelled`, `rate_limited`, `usage_limited`) — use a parameterized test over all values.
...
7. Add unit tests in `packages/daemon/tests/unit/space/space-runtime-completion.test.ts` for the runtime ordering and orchestration task auto-completion:
   ...
   - Test: when the orchestration task is `pending` (not yet `in_progress`) at end-node auto-completion time, the runtime does not throw and the run still transitions to `completed` (the orchestration task is left as-is).
...
3. Add unit tests in `packages/daemon/tests/unit/space/space-workflow.test.ts`:
   ...
   - Test: RPC `workflow.create` request with `endNodeId` results in the manager receiving `endNodeId` (regression guard against future field whitelisting in the RPC handler).
