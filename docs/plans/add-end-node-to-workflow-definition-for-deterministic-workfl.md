...
4. **WorkflowEditor (form-based)** (`packages/web/src/components/space/WorkflowEditor.tsx`):
   ...
   - **Note:** The form-based editor currently does not expose a `startNodeId` selector either (it hardcodes `startNodeId: stepIds[0]`). Follow the same pattern for `endNodeId` — default to the last step in the list (`stepIds[stepIds.length - 1]`). Add a code comment: `// Heuristic: defaults to last step — may be wrong if user reorders steps; use the visual editor for explicit control`. The "Set as End Node" interactive UI is only available in the visual editor; this asymmetry is acceptable since the visual editor is the primary editing mode for workflow topology.
5. **Tests:**
