# Milestone 5: Task Agent Refactoring
...
- Wire new dependencies (ChannelRouter, CompletionDetector) into Task Agent
- Tests

## Tasks

### Task 5.1: Update Task Agent System Prompt
...
2. In `buildTaskAgentInitialMessage()`:
   - Include channel information in the initial message (channels from the workflow)
...
- Prompt includes guidance for using channel-based messaging and monitoring completion
...
### Task 5.2: Add report_workflow_done Tool
...
### Task 5.3: Wire New Dependencies into Task Agent

**Description**: Wire the `ChannelRouter` and `CompletionDetector` into the Task Agent's MCP server configuration.
...
**Acceptance Criteria**:
- Task Agent MCP server has access to channel routing
- Task Agent can query completion status
- No circular dependencies

**Dependencies**: Tasks 5.2, 3.3

**Agent Type**: coder

---

### Task 5.4: Verify Task Agent MCP Server Configuration

**Description**: Verify the Task Agent's MCP server is clean and consistent after the `advance_workflow` removal (done in Task 3.5).
...
### Task 5.5: Tests for Refactored Task Agent
...
3. Create `packages/daemon/tests/unit/space/task-agent-collaboration.test.ts`:
   ...
   - Test gate-blocked flow: agent tries message, gate blocks, Task Agent escalates
...
- **System prompt changes** (Task 5.1): Prompt changes take effect on the next Task Agent session creation. Reverting is instant — no persisted state depends on it.
- **report_workflow_done** (Task 5.2): New tool. Can be removed from the tool list without side effects.
- **MCP wiring** (Task 5.3): New dependencies injected into MCP server config. Reverting removes the injections — step agent tools work without them.
