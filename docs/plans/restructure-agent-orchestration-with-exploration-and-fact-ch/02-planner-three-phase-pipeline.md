...
Restructure the planner agent to use a 3-phase sequential pipeline: planner-explorer -> planner-fact-checker -> plan-writer. The planner orchestrates all three phases, passing accumulated context forward. The plan-writer's broken "spawn Explore agents" instruction is removed since sub-agents cannot spawn further sub-agents.
...
### Task 2.1: Add planner-explorer and planner-fact-checker sub-agent definitions
...
2. Define `buildPlannerFactCheckerAgentDef(): AgentDefinition` — web research and validation agent
   ...
   - Rationale: The fact-checker focuses exclusively on web-based validation. It does NOT have Read/Grep/Glob — those are the explorer's responsibility. The fact-checker receives explorer findings as context and validates them against external sources (API docs, library changelogs, best practices). If the fact-checker needs to verify a specific version in `package.json`, that version should already be in the explorer findings.
   - Model: `'inherit'`
...
- Fact-checker has only `['WebSearch', 'WebFetch']` — no codebase tools
...
**Breaking change:** The current plan-writer has Task/TaskOutput/TaskStop tools and its test (`planner-agent.test.ts` line 302-307, "has Task tool for spawning Explore sub-agents") asserts their presence. This is intentionally broken — removing these tools is the fix. The test must be updated to assert the opposite.
...
**Description:** Update `buildPlannerSystemPrompt()` to describe the 3-phase pipeline. The planner now orchestrates: (1) spawn `planner-explorer` to gather codebase context, (2) spawn `planner-fact-checker` with explorer findings, (3) spawn `plan-writer` with both sets of findings. The planner passes accumulated context forward at each phase.

**Subtasks:**
1. Replace the current Phase 1 description (single plan-writer spawn) with the 3-phase pipeline:
   - Phase 1: Spawn `planner-explorer` with goal context, collect `---EXPLORER_FINDINGS---`
   - Phase 2: Spawn `planner-fact-checker` with goal context + explorer findings, collect `---FACT_CHECK_RESULT---`
...
3. Add a concrete prompt construction template for each phase transition. Example for Phase 3 (spawning plan-writer):
   ```
   Task(prompt: "Goal: <goal title>\n\nDescription: <goal description>\n\n## Explorer Findings\n\n<verbatim ---EXPLORER_FINDINGS--- block>\n\n## Fact-Check Results\n\n<verbatim ---FACT_CHECK_RESULT--- block>\n\n<any additional planner instructions>")
   ```
4. Add error-handling guidance: if `planner-explorer` fails or times out, proceed to Phase 3 (plan-writer) directly with only the goal context — the plan-writer has its own Read/Grep/Glob tools. If `planner-fact-checker` fails, proceed to Phase 3 with explorer findings only and note that fact-checking was skipped
5. Preserve WebSearch/WebFetch in planner's own tools for pre-planning verification
6. Update Phase 2 (task creation after approval) to remain unchanged
7. Keep the existing feedback handling ("If the Leader sends feedback...")

**Acceptance Criteria:**
- Planner system prompt describes 3-phase pipeline with explicit phase ordering
- Context-passing pattern is clearly documented with prompt construction template
- Error-handling for sub-agent failures is specified (graceful degradation, not abort)
...
**Description:** Update `createPlannerAgentInit()` to include `planner-explorer`, `planner-fact-checker`, and `plan-writer` in the `agents` map. The planner always uses the agent/agents pattern with all three sub-agents available.

**Subtasks:**
1. Add `planner-explorer` (from `buildPlannerExplorerAgentDef()`) to the agents map
2. Add `planner-fact-checker` (from `buildPlannerFactCheckerAgentDef()`) to the agents map
...
**Acceptance Criteria:**
- `createPlannerAgentInit()` returns init with agents map containing: Planner, `planner-explorer`, `planner-fact-checker`, `plan-writer`
...
- Tests verify plan-writer no longer references Explore sub-agents and no longer has Task tools
