...
2. Wire form submission to `spaceStore.createTask()` which calls `spaceTask.create` RPC. Note: `spaceStore.spaceId.value` must be set (it is set when SpaceIsland loads a space); the dialog is only rendered within the space detail view so this is guaranteed
