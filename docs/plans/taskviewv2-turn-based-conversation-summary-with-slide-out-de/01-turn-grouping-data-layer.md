...
4. Extract the group message fetching and parsing logic from `TaskConversationRenderer.tsx` into `packages/web/src/hooks/useGroupMessages.ts`:
   - **Data flow clarification**: The RPC `task.getGroupMessages` returns `GroupMessage` objects with fields `id`, `groupId`, `sessionId`, `role`, `messageType`, `content`, `createdAt`, and `cursor` (see `TaskConversationRenderer.tsx` lines 44-52 and `task-handlers.ts` lines 756-765). **Important**: `GroupMessage.id` is a synthetic sequential integer (`idx + 1`) re-assigned on every page fetch — it is NOT a stable DB identifier and must never be used for stable keying. The `cursor` field is essential for pagination (`oldestCursorRef` tracking in load-older logic). The `parseGroupMessage()` function transforms each `GroupMessage` into an `SDKMessage`, attaching `_taskMeta` on specific synthetic messages (status, leader_summary, etc.). The hook consumes `GroupMessage` from the RPC and returns the parsed `SDKMessage` results.
   - **Note on synthetic message IDs**: For status/leader_summary synthetic messages, `_taskMeta.turnId` is derived as `status-${msg.id}` or `leader_summary-${msg.id}` where `msg.id` is the ephemeral integer. This means RuntimeMessage items have inherently unstable keys across refetches — acceptable since they are not TurnBlocks, but the hook must not rely on these for stable keying.
