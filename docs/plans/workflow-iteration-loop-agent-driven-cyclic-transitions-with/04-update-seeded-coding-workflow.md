...
4. **Update `seedBuiltInWorkflows()` to forward new fields:**
   - The transition mapping at line ~237-242 manually constructs new transition objects: `{ from, to, condition, order }`. Add `isCyclic: t.isCyclic` to this mapping. Without this, the `isCyclic: true` flag on the Verify→Plan transition will be silently dropped at seeding time, defeating iteration tracking for seeded workflows.
   - The `workflowManager.createWorkflow()` call at line ~246-255 must pass `maxIterations: template.maxIterations`. Without this, `maxIterations: 3` will be set on the template object but silently dropped on persist, and seeded runs will use the DB default of 5.
5. Update the step ID constants and transition wiring to reflect the new graph: Plan -> Code -> Verify -[failed]-> Plan, Verify -[passed]-> Done.
6. Verify the `neededRoles` set in `seedBuiltInWorkflows` will include any new agent role used by the Verify step.
