...
3. Add `isCyclic?: boolean` to `ExportedWorkflowTransition` (line ~739 of `space.ts`). This is a separate interface that does NOT extend `WorkflowTransition` — the field must be added explicitly. Also update the export logic in `packages/daemon/src/lib/space/export-format.ts` (line ~192-198, note: no `export/` subdirectory) to include `isCyclic` when building the exported transition (e.g., `if (t.isCyclic !== undefined) exported.isCyclic = t.isCyclic;`). Also update the Zod schemas in the same file:
   - The `workflowConditionSchema` at line ~38 hardcodes `z.enum(['always', 'human', 'condition'])`. Add `'task_result'` to this enum so workflows with `task_result` transitions can be imported without Zod validation errors.
   - The `exportedWorkflowTransitionSchema` at lines ~60-65 must add `isCyclic: z.boolean().optional()`. Without this, Zod will silently strip `isCyclic` from imported transitions (Zod strips unknown keys by default), breaking the export/import round-trip.
