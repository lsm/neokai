...
- **Invariant**: at most one active execution per recurring mission. Enforced at two levels: (1) **DB-level** — partial unique index on `mission_executions(goal_id) WHERE status = 'running'` so concurrent inserts fail with a unique constraint violation, and (2) **app-level** — check no `mission_executions` row with `status = 'running'` for this goal before creating a new one. On unique-conflict, log and skip (idempotent). Wrap the trigger path (create execution row + advance `next_run_at`) in a single transaction.
...
- **Daemon restart catch-up**: If `next_run_at` is in the past, fire once immediately. Calculate next `next_run_at` from current time (skip missed intervals). **Ordering**: zombie group recovery (`recoverZombieGroups()`) runs before the scheduler tick, so any in-flight execution from before the restart will be recovered first. The scheduler's "no active execution" check then correctly prevents a duplicate catch-up trigger.
...
- **Single atomic write path**: All recurring-mission task linkage MUST go through a single helper — `GoalManager.linkTaskToExecution(goalId, executionId, taskId)` — which atomically updates both `mission_executions.task_ids` (append taskId to JSON array) and `goals.linked_task_ids` (append taskId) in one transaction. For non-recurring missions, the existing `GoalManager.linkTaskToGoal()` continues to work unchanged. All current call sites that link tasks to goals — planner draft-task creation (`room-runtime.ts`), room agent `create_task(goal_id)` (`room-agent-tools.ts`), RPC handler `goal.linkTask` (`goal-handlers.ts`), and the restart safety-net relinking path — must be updated to call `linkTaskToExecution()` instead when the goal is recurring and has an active execution. This prevents the two stores from drifting apart.
...
- All task-linking call sites use `linkTaskToExecution()` for recurring missions (atomic dual-write to `mission_executions.task_ids` + `goals.linked_task_ids`)
