...
- **Single atomic write path**: All recurring-mission task linkage MUST go through a single helper — `GoalManager.linkTaskToExecution(goalId, executionId, taskId)` — which atomically updates both `mission_executions.task_ids` (append taskId to JSON array) and `goals.linked_task_ids` (append taskId) in one transaction. For non-recurring missions, the existing `GoalManager.linkTaskToGoal()` continues to work unchanged. All current call sites that link tasks to goals — planner draft-task creation (`room-runtime.ts`), room agent `create_task(goal_id)` (`room-agent-tools.ts`), and the restart safety-net relinking path — must be updated to call `linkTaskToExecution()` instead when the goal is recurring and has an active execution. This prevents the two stores from drifting apart.
