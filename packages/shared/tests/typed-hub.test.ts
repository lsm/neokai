...
describe('TypedHub', () => {
	...
	describe('basic pub/sub', () => {
		it('should publish and receive events', async () => {
			...
			await hub.publish('session.created', { sessionId: 'test-1', title: 'Test Session' });
		...
		it('should support multiple subscribers', async () => {
			...
			await hub.publish('session.created', { sessionId: 'multi-test', title: 'Multi' });
			await new Promise((resolve) => setTimeout(resolve, 20));
		...
		it('should support unsubscribe', async () => {
			...
			await hub.publish('session.created', { sessionId: 'before', title: 'Before' });
			await new Promise((resolve) => setTimeout(resolve, 20));

			unsubscribe();

			await hub.publish('session.created', { sessionId: 'after', title: 'After' });
			await new Promise((resolve) => setTimeout(resolve, 20));
		...
		it('should support once() for one-time subscriptions', async () => {
			...
			await hub.publish('session.created', { sessionId: 'first', title: 'First' });
			await hub.publish('session.created', { sessionId: 'second', title: 'Second' });
	...
	describe('session-scoped subscriptions', () => {
		it('should filter events by sessionId', async () => {
			...
			await hub.publish('message.sent', { sessionId: 'session-1', content: 'msg1' });
			await hub.publish('message.sent', { sessionId: 'session-2', content: 'msg2' });
			await hub.publish('message.sent', { sessionId: 'session-3', content: 'msg3' });
			await new Promise((resolve) => setTimeout(resolve, 30));
		...
		it('should support session-scoped once()', async () => {
			...
			await hub.publish('message.sent', { sessionId: 'other-session', content: 'other1' });
			await hub.publish('message.sent', { sessionId: 'other-session', content: 'other2' });

			// This should be received (and unsubscribe)
			await hub.publish('message.sent', { sessionId: 'target-session', content: 'target1' });

			// This should be missed (already unsubscribed)
			await hub.publish('message.sent', { sessionId: 'target-session', content: 'target2' });
	...
	describe('multi-participant communication', () => {
		it('should create participants connected to same bus', async () => {
			...
			await hub.publish('session.created', { sessionId: 'from-hub', title: 'From Hub' });
			await new Promise((resolve) => setTimeout(resolve, 20));
	...
	describe('error handling', () => {
		it('should throw when publishing before initialization', async () => {
			const uninitializedHub = new TypedHub<TestEventMap>({ name: 'uninitialized' });

			await expect(
				uninitializedHub.publish('session.created', { sessionId: 'test', title: 'Test' })
	...
	describe('multiple event types', () => {
		it('should handle multiple event types independently', async () => {
			...
			await hub.publish('session.created', { sessionId: 'another', title: 'Another' });
			await new Promise((resolve) => setTimeout(resolve, 30));
	...
	describe('cleanup', () => {
		it('should cleanup subscriptions on close', async () => {
			...
			await hub.publish('session.created', { sessionId: 'before-close', title: 'Before' });
			await new Promise((resolve) => setTimeout(resolve, 20));
			...
			await expect(
				hub.publish('session.created', { sessionId: 'after-close', title: 'After' })
			).rejects.toThrow();
