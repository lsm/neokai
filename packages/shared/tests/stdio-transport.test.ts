...
import {
  StdioTransport,
  createStdioPair,
} from "../src/message-hub/stdio-transport";
import {
  MessageType,
  createCallMessage,
  createResultMessage,
...
describe("StdioTransport", () => {
  describe("createStdioPair", () => {
    ...
    it("should send message from client to server", async () => {
      ...
      expect((received[0] as { data: { hello: string } }).data.hello).toBe(
        "world",
      );
    });
    ...
    it("should send message from server to client", async () => {
      ...
      expect((received[0] as { type: MessageType }).type).toBe(
        MessageType.RESULT,
      );
    });
    ...
    it("should support bidirectional communication", async () => {
      ...
      expect((serverReceived[0] as { method: string }).method).toBe(
        "echo.request",
      );

      expect(clientReceived.length).toBe(1);
      expect((clientReceived[0] as { requestId: string }).requestId).toBe(
        request.id,
      );
    });
  ...
  describe("error handling", () => {
    ...
    it("should handle invalid JSON gracefully", async () => {
      ...
      expect(received.length).toBe(1);
      expect((received[0] as { data: { valid: boolean } }).data.valid).toBe(
        true,
      );
