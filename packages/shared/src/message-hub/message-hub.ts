...
export class MessageHub {
  ...
  private persistedSubscriptions: Map<string, PersistedSubscription> =
    new Map();
  ...
  registerTransport(transport: IMessageTransport): UnsubscribeFn {
    if (this.transport) {
      throw new Error(
        "Transport already registered. Call unregisterTransport() first.",
      );
    }
  ...
  async subscribe<TData = unknown>(
    ...
    if (this.isConnected()) {
      ...
      try {
        ...
        await ackPromise;
        this.logDebug(
          `Subscribed to: ${method} (session: ${sessionId}) - ACK received`,
        );
      } catch (error) {
        // Cleanup on failure
    ...
    return async () => {
      ...
      if (this.isConnected()) {
        ...
        try {
          ...
          await ackPromise;
          this.logDebug(
            `Unsubscribed from: ${method} (session: ${sessionId}) - ACK received`,
          );
        } catch (error) {
  ...
  subscribeOptimistic<TData = unknown>(
    ...
    return () => {
      ...
      this.logDebug(
        `Unsubscribed (optimistic) from: ${method} (session: ${sessionId})`,
      );
  ...
  private async handleIncomingMessage(message: HubMessage): Promise<void> {
    ...
    if (typeof message.sequence === "number" && this.warnOnSequenceGap) {
      const clientId = (message as import("./protocol").HubMessageWithMetadata)
        .clientId;
    ...
    try {
      if (message.type === MessageType.PING) {
        ...
        isSubscribedMessage(message) ||
        isUnsubscribedMessage(message)
      ) {
  ...
  private handleSubscriptionResponse(message: HubMessage): void {
    ...
    if (
      message.type === MessageType.SUBSCRIBED ||
      message.type === MessageType.UNSUBSCRIBED
    ) {
      const action =
        message.type === MessageType.SUBSCRIBED ? "SUBSCRIBE" : "UNSUBSCRIBE";
  ...
  private handleResponse(message: HubMessage): void {
    ...
    if (!pending) {
      this.logDebug(
        `Response for unknown request: ${requestId} (method: ${message.method})`,
      );
      return;
    }
      ...
      for (const handler of handlers) {
        try {
          ...
          if (this.stopOnEventHandlerError) {
            ...
            log.error(
              `Event handler failed for ${message.method} (stopping):`,
              err,
            );
            break;
          }
  ...
  private async handleSubscribe(message: HubMessage): Promise<void> {
    ...
    if (!clientId) {
      log.error(
        "SUBSCRIBE without clientId - transport must add clientId to messages",
      );
      return;
    }
    ...
    try {
      ...
      this.logDebug(
        `Client ${clientId} subscribed to ${message.sessionId}:${message.method}`,
      );
  ...
  private async handleUnsubscribe(message: HubMessage): Promise<void> {
    ...
    try {
      ...
      this.logDebug(
        `Client ${clientId} unsubscribed from ${message.sessionId}:${message.method}`,
      );
  ...
  private async sendMessage(message: HubMessage): Promise<void> {
    ...
    if (this.router && message.type === MessageType.EVENT) {
      ...
      this.logDebug(
        `Routed event: ${result.sent}/${result.totalSubscribers} delivered`,
      );
      return;
    }
   ...
   */
  private async sendResponseToClient(
    message: HubMessage,
    clientId?: string,
  ): Promise<void> {
    // Add sequence number for ordering guarantees
    ...
    if (this.router && clientId) {
      ...
      if (!success) {
        log.warn(
          `Failed to send response to client ${clientId}, falling back to broadcast`,
        );
  ...
  forceResubscribe(): void {
    this.logDebug(
      "Force resubscribing all subscriptions (connection validation)",
    );
    this.resubscribeAll();
  }
  ...
  private resubscribeAll(): void {
    ...
    try {
      ...
      for (const [_subId, { method, handler, options }] of this
        ...
        if (this.isConnected() && !this.inFlightSubscriptions.has(subKey)) {
          ...
          const ackPromise = new Promise<{ method: string; success: boolean }>(
            (resolve) => {
              const timer = setTimeout(() => {
                this.pendingSubscribes.delete(subId);
                this.inFlightSubscriptions.delete(subKey); // Clean up on timeout
                this.logDebug(
                  `Subscription ACK timeout for ${method} (session: ${sessionId})`,
                );
          ...
          this.sendMessage(subscribeMsg).catch((error) => {
            log.error(
              `Failed to send SUBSCRIBE for ${method} (session: ${sessionId}):`,
              error,
            );
          ...
          this.logDebug(
            `Skipping duplicate SUBSCRIBE for ${method} (session: ${sessionId})`,
          );
        }
    ...
    if (subscriptionPromises.length > 0) {
      Promise.allSettled(subscriptionPromises).then((results) => {
        ...
        if (failed > 0) {
          ...
          this.logDebug(
            `Resubscription completed: all ${succeeded} ACKs received`,
          );
        }
      });
    }
