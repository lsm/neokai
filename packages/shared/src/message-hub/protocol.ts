...
export function isResponseMessage(
  msg: HubMessage,
): msg is ResultMessage | ErrorMessage {
...
export function createCallMessage(
  params: CreateCallMessageParams,
): CallMessage {
...
export function createResultMessage(
  params: CreateResultMessageParams,
): ResultMessage {
...
export function createErrorMessage(
  params: CreateErrorMessageParams,
): ErrorMessage {
  const { method, error: errorParam, sessionId, requestId, id } = params;
  const errorMessage =
    typeof errorParam === "string" ? errorParam : errorParam.message;
...
export function createEventMessage(
  params: CreateEventMessageParams,
): EventMessage {
  const { method, data, sessionId, id } = params;
  return {
    id: id || generateUUID(),
...
export function createSubscribeMessage(
  params: CreateSubscribeMessageParams,
): SubscribeMessage {
...
export function createSubscribedMessage(
  params: CreateSubscribedMessageParams,
): SubscribedMessage {
...
export function isValidMessage(msg: unknown): msg is HubMessage {
  ...
  if (
    ...
    !Object.values(MessageType).includes(m.type as MessageType)
  ) {
    return false;
  }
