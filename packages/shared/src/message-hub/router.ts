...
export class MessageHubRouter {
  ...
  registerConnection(connection: ClientConnection): string {
    ...
    if (existing) {
      this.log(
        `Client already registered: ${existing.clientId}, returning existing ID`,
      );
  ...
  subscribe(sessionId: string, method: string, clientId: string): void {
    ...
    if (sessionId.includes(":")) {
      throw new Error(
        "SessionId cannot contain colon character (reserved for internal use)",
      );
    }
    ...
    if (method.includes(":")) {
      throw new Error(
        "Method cannot contain colon character (reserved for internal use)",
      );
    }
  ...
  sendToClient(clientId: string, message: HubMessage): boolean {
    ...
    try {
      ...
      this.logger.error(
        `[MessageHubRouter] Failed to send to client ${clientId}:`,
        error,
      );
      return false;
    }
  ...
  broadcast(message: HubMessage): {
    ...
    try {
      ...
      this.logger.error(
        `[MessageHubRouter] Failed to serialize broadcast message:`,
        error,
      );
    ...
    for (const client of this.clients.values()) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to broadcast to client ${client.clientId}:`,
          error,
        );
        failedCount++;
      }
