...
/**
 * Role of an agent within a Space workflow
 */
export type SpaceAgentRole = 'worker' | 'reviewer' | 'orchestrator';

/**
 * A custom agent defined within a Space.
 * Agents have configurable models, tools, system prompts, and roles.
 */
export interface SpaceAgent {
	...
	spaceId: string;
	/** Human-readable name (unique within the Space) */
	name: string;
	/** Short description of this agent's purpose */
	description: string;
	/** Model identifier for this agent (falls back to Space default if undefined) */
	model?: string;
	/** Provider identifier (e.g. 'anthropic', 'glm') */
	provider?: string;
	/** List of tool names this agent is allowed to use */
	tools: string[];
	/** System prompt for this agent */
	systemPrompt: string;
	/** Role of this agent in workflows */
	role: SpaceAgentRole;
