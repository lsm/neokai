 ...
 */
export const logger = createLogger('liuboer');
...
export const loggers = {
	messagehub: createLogger('liuboer:messagehub'),
	transport: createLogger('liuboer:transport'),
	session: createLogger('liuboer:session'),
	worktree: createLogger('liuboer:worktree'),
	state: createLogger('liuboer:state'),
	daemon: createLogger('liuboer:daemon'),
	cli: createLogger('liuboer:cli'),
	web: createLogger('liuboer:web'),
};

/**
 * Utility to silence a namespace temporarily (useful for tests)
 * Returns a function to restore the previous filter
 */
export function silenceNamespace(namespace: string): () => void {
	const previousExclude = [...globalConfig.excludeFilter];
	globalConfig.excludeFilter.push(namespace);

	// Clear all logger caches
	return () => {
		globalConfig.excludeFilter = previousExclude;
	};
}

/**
 * Utility to set log level temporarily (useful for tests)
 * Returns a function to restore the previous level
 */
export function setLogLevel(level: LogLevel): () => void {
	const previousLevel = globalConfig.level;
	globalConfig.level = level;

	return () => {
		globalConfig.level = previousLevel;
	};
}
...
export function isDevelopment(): boolean {
	return getEnv('NODE_ENV') === 'development' || !getEnv('NODE_ENV');
}
