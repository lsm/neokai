...
import type { Session, AuthMethod, ContextInfo, MessageContent, MessageImage } from './types.ts';
...
export interface EventMap {
	...
	'settings:updated': { sessionId: string; settings: import('./types/settings.ts').GlobalSettings };
...
export class EventBus {
	...
	on<K extends keyof EventMap>(
		...
		if (options?.sessionId) {
			this.log(`Registered session-scoped handler for: ${eventKey} (session: ${sessionId})`);
		} else {
		...
		return () => {
			...
			if (options?.sessionId) {
				this.log(`Unregistered session-scoped handler for: ${eventKey} (session: ${sessionId})`);
			} else {
