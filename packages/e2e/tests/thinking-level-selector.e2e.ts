...
test.describe("Thinking Level Selector", () => {
  ...
  test("should display thinking level button with default Auto level", async ({
    page,
  }) => {
  ...
  test("should open dropdown when clicking thinking level button", async ({
    page,
  }) => {
    // Create a new session
    await page
  ...
  test("should select Think 16k level and persist", async ({ page }) => {
    ...
    // Button title should update
    await expect(thinkingButton).toHaveAttribute(
      "title",
      "Thinking: Think 16k",
    );
  });
  ...
  test("should show current level indicator in dropdown", async ({ page }) => {
    ...
    const think16kOption = page.locator(
      'button:has-text("Think 16k (current)")',
    );
  ...
  test("should close model dropdown when opening thinking dropdown", async ({
    page,
  }) => {
    // Create a new session
    await page
  ...
  test("should persist thinking level after page refresh", async ({ page }) => {
    ...
    await expect(refreshedThinkingButton).toHaveAttribute(
      "title",
      "Thinking: Think 32k",
      {
        timeout: 10000,
