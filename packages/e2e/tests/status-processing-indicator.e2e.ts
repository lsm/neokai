...
test.describe("Processing Indicator in Sidebar", () => {
  ...
  test("should show pulsing indicator when session is processing", async ({
    page,
  }) => {
  ...
  test("should not show pulsing indicator when session is idle", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
  ...
  test("should show correct phase colors during processing", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
    ...
    const textarea = page.locator('textarea[placeholder*="Ask"]').first();
    await textarea.fill(
      "Think carefully and explain the Pythagorean theorem step by step.",
    );
    await page.keyboard.press("Meta+Enter");
  ...
  test("should return to idle state after processing completes", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
