...
test.describe("Context Dropdown", () => {
  ...
  test("should display context percentage after message exchange", async ({
    page,
  }) => {
    ...
    const contextPercentage = page
      .locator('.text-xs.font-medium:has-text("%")')
      .first();
  ...
  test("should show context category breakdown in dropdown", async ({
    page,
  }) => {
    // Create a new session - use specific selector for the primary New Session button
    const newSessionButton = page.getByRole("button", {
    ...
    const percentageCount = await dropdownContent
      .getByText(/\d+\.\d+%/, { exact: false })
      .count();
