...
test.describe("Recent Sessions Home Page", () => {
  ...
  test("should show Recent Sessions section after creating a session", async ({
    page,
  }) => {
    // Create a session
    await page
  ...
  test("should navigate to session when clicking session card", async ({
    page,
  }) => {
    // Create a session
    await page
    ...
    await expect(
      page.locator('textarea[placeholder*="Ask"]').first(),
    ).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("should show relative time for sessions", async ({ page }) => {
    ...
    const timePatterns = page.locator(
      "text=/just now|seconds? ago|minutes? ago|hours? ago/i",
    );
    await expect(timePatterns.first()).toBeVisible({ timeout: 5000 });
  });

  test("should display correct subtitle based on session state", async ({
    page,
  }) => {
    ...
    if (hasRecentSessions) {
      await expect(
        page.locator("text=Continue where you left off"),
      ).toBeVisible();
    } else {
      await expect(
        page.locator("text=Create a new session to get started"),
      ).toBeVisible();
    }
  });
