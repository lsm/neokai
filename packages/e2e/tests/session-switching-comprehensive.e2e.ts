...
test.describe('Session Switching - Comprehensive Coverage', () => {
  test('should handle rapid session switching without cleanup errors', async ({ page }) => {
    ...
    const cleanupErrors = consoleErrors.filter(err =>
      err.includes('cleanup is not a function') ||
    ...
    for (const sessionId of sessionIds) {
      await page.click(`[data-session-id="${sessionId}"]`);
      await expect(page.getByPlaceholder(/Ask, search, or make anything/i)).toBeVisible({ timeout: 5000 });
    }
  ...
  test('should maintain correct session context after multiple rapid switches', async ({ page }) => {
    await setupMessageHubTesting(page);
