...
test.describe("Message Pagination", () => {
  ...
  test("should handle conversation with multiple exchanges", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
  ...
  test("should display Load More button when history exceeds page size", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
    ...
    const buttonAfterMessage = await loadMoreButton
      .isVisible()
      .catch(() => false);
