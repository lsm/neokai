...
test.describe("Interrupt Button", () => {
  ...
  test("should have clickable stop button (not disabled) when agent is processing", async ({
    ...
    const isGrayedOut = await stopButton.evaluate((el) => {
      ...
      return (
        classes.includes("cursor-not-allowed") || classes.includes("opacity-50")
      );
    });
  ...
  test("should interrupt agent when stop button is clicked", async ({
    page,
  }) => {
    // Create a session
    await page
  ...
  test("should interrupt agent when Escape key is pressed", async ({
    page,
  }) => {
    // Create a session
    await page
    ...
    const messageInput = await waitForElement(page, "textarea");
    await messageInput.fill(
      "Explain the history of the internet in detail with examples.",
    );
    await page.click('[data-testid="send-button"]');
  ...
  test("should show loading spinner while interrupting", async ({ page }) => {
    ...
    const isDisabledDuringInterrupt = await stopButton
      .isDisabled()
      .catch(() => false);
  ...
  test("should transition from send to stop and back to send", async ({
    page,
  }) => {
    // Create a session
    await page
  ...
  test.skip("should handle rapid interrupt attempts gracefully", async ({
    page,
  }) => {
  ...
  test("Escape key should work even when textarea is disabled", async ({
    page,
  }) => {
    // Create a session
    await page
