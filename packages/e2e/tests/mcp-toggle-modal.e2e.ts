...
test.describe("MCP Toggle - Tools Modal", () => {
  ...
  test("should open Tools modal from session options menu", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('h3:has-text("System Prompt")').first(),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("Setting Sources")').first(),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("MCP Servers")').first(),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("Liuboer Tools")').first(),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("SDK Built-in")').first(),
    ).toBeVisible();
  });
  ...
  test("should show MCP servers section with servers from settings", async ({
    page,
  }) => {
    await openToolsModal(page);
  ...
  test("should enable Save button when MCP toggle changes", async ({
    page,
  }) => {
    await openToolsModal(page);
  ...
  test("should remove server from disabledMcpjsonServers when re-enabled", async ({
    page,
  }) => {
    await openToolsModal(page);
