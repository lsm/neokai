...
test.describe("Processing State Display", () => {
  ...
  test("should show processing indicator when sending message", async ({
    page,
  }) => {
  ...
  test("should show thinking state during initial processing", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
  ...
  test("should show streaming state during response generation", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
  ...
  test("should show different colored indicators for different phases", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
    ...
    const colorClasses = await processingIndicator.evaluate(
      (el) => el.className,
    );
