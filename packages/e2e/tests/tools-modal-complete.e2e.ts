...
test.describe("Tools Modal - Complete", () => {
  ...
  test.skip("should show Setting Sources section with checkboxes", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
    ...
    // Wait for modal
    await expect(
      page.locator('h2:has-text("Tools"), h3:has-text("Tools")').first(),
    ).toBeVisible({
      timeout: 5000,
    });
  ...
  test.skip("should enable Save button when settings change", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
  ...
  test.skip("should close modal with Cancel button without saving", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
    ...
    const modalTitle = page
      .locator('h2:has-text("Tools"), h3:has-text("Tools")')
      .first();
  ...
  test.skip("should toggle Claude Code Preset", async ({ page }) => {
    ...
    const presetRow = page
      .locator('div:has-text("Claude Code Preset")')
      .first();
    const toggle = presetRow
      .locator('button[role="switch"], input[type="checkbox"]')
      .first();
