...
export const WORKSPACE_PATH = join(
  process.cwd(),
  "..",
  "cli",
  "tmp",
...
export const SETTINGS_LOCAL_PATH = join(
  WORKSPACE_PATH,
  ".claude",
  "settings.local.json",
);
...
export async function openToolsModal(page: Page): Promise<void> {
  ...
  const sessionOptionsButton = page
    .locator('button[title="Session options"]')
    .first();
...
export async function getMcpServerNames(page: Page): Promise<string[]> {
  ...
  const mcpSection = page
    .locator('h3:has-text("MCP Servers")')
    .locator("..")
    .first();
