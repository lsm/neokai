...
test.describe.serial('Git Worktree Integration', () => {
...
test.describe.serial('Worktree Manual Cleanup', () => {
	// Use system temp directory instead of process.cwd() to avoid worktree issues
	...
	test('should cleanup orphaned worktrees via RPC call', async ({ page }) => {
		...
		await execAsync(`git worktree remove "${worktreePath}"`, { cwd: testRepoPath });
		await execAsync(`git branch -D session/${orphanedId}`, { cwd: testRepoPath });
	});
});
