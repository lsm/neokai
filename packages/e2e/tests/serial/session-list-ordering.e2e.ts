...
test.describe.serial("Session List Ordering", () => {
  ...
  test("should show newly created session at top of session list", async ({
    page,
  }) => {
    ...
    const firstSessionInList = await sessionItems
      .first()
    ...
    const firstListItemId = await sessionItems
      .first()
      .getAttribute("data-session-id");
    const secondListItemId = await sessionItems
      .nth(1)
      .getAttribute("data-session-id");
  ...
  test("should maintain correct order after creating multiple sessions", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < 3; i++) {
      const sessionId = await sessionItems
        .nth(i)
        .getAttribute("data-session-id");
