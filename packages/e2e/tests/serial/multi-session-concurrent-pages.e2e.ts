...
test.describe("Multi-Session Concurrent Pages (Skipped - Flaky)", () => {
  test.skip("should handle multiple sessions independently", async ({
    browser,
  }) => {
    ...
    try {
      ...
      const messagePromises = pages.map(async (page, index) => {
        ...
        return waitForMessageProcessed(
          page,
          `Message from session ${index + 1}`,
        );
      });
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        for (let j = 0; j < pages.length; j++) {
          if (i !== j) {
            ...
            await expect(
              page.locator(`text="${otherMessage}"`),
            ).not.toBeVisible();
          }
        }
      }
  ...
  test.skip("should maintain separate conversation contexts", async ({
    browser,
  }) => {
    // TODO: Multi-page concurrent tests are flaky and can crash the server
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        const messageInput = await waitForElement(page, "textarea");
        await messageInput.fill(
          "What is my name and what topic did I mention?",
        );
        await page.click('[data-testid="send-button"]');
  ...
  test.skip("should handle concurrent messages across sessions", async ({
    browser,
  }) => {
    // TODO: Multi-page concurrent tests are flaky and can crash the server
    const context = await browser.newContext();
    ...
    try {
      ...
      await expect(
        page1.locator('text="Concurrent message 2"'),
      ).not.toBeVisible();

      await expect(page2.locator('text="Concurrent message 2"')).toBeVisible();
      await expect(
        page2.locator('text="Concurrent message 1"'),
      ).not.toBeVisible();
  ...
  test.skip("should handle message queue independently per session", async ({
    browser,
  }) => {
    // TODO: Multi-page concurrent tests are flaky and can crash the server
    const pages = await createMultiplePages(browser, 2);
