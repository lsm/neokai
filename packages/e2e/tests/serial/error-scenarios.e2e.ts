...
import {
  ...
  restoreNetwork,
} from "../helpers/interruption-helpers";
...
test.describe("Error Scenarios", () => {
  ...
  test("should prevent message send when connection is lost", async ({
    page,
  }) => {
    // Create a session
    await page
  ...
  test.skip("should handle network disconnection during message send", async ({
    page,
  }) => {
  ...
  test("should recover from temporary WebSocket disconnection", async ({
    page,
  }) => {
    // Track reconnection
    await page.evaluate(() => {
    ...
    await expect(
      page.locator('.text-green-400:has-text("Online")').first(),
    ).toBeVisible({
      timeout: 10000,
    });
