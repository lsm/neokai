...
test.describe("Message Send and Receive", () => {
  ...
  test("should successfully send a message and receive response", async ({
    page,
  }) => {
    ...
    const statusText = page.locator(
      "text=/Starting|Thinking|Streaming|Processing/i",
    );
  ...
  test("should handle message sending state transitions correctly", async ({
    page,
  }) => {
    // Create a new session
    const newSessionBtn = page.getByRole("button", {
    ...
    await expect(
      page.locator("text=/Starting|Thinking|Streaming|Processing/i"),
    ).toBeVisible({
      timeout: 5000,
    });
  ...
  test("should display message immediately in UI (optimistic update)", async ({
    page,
  }) => {
    // Create a new session
    const newSessionBtn = page.getByRole("button", {
  ...
  test("should handle consecutive messages correctly", async ({ page }) => {
    ...
    for (const msg of messages) {
      ...
      const processingIndicator = page
        .locator("text=/Sending|Processing|Queued/i")
        .first();
  ...
  test("should recover from send failures", async ({ page }) => {
    ...
    const nextSendButton = await waitForElement(
      page,
      '[data-testid="send-button"]',
    );
    ...
    await expect(
      page.locator('text="Second message after recovery"').first(),
    ).toBeVisible({
      timeout: 5000,
    });
    await expect(messageInput).toBeEnabled({ timeout: 15000 });
