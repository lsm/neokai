...
test.describe("Multi-Session Rapid Operations", () => {
    ...
    for (const session of sessionData) {
      ...
      await expect(
        page
          ...
          .filter({ hasText: session.message }),
      ).toBeVisible();
      ...
      for (const otherSession of sessionData) {
        if (otherSession.id !== session.id) {
          await expect(
            page
              .locator('[data-testid="user-message"]')
              .filter({ hasText: otherSession.message }),
          ).not.toBeVisible();
        }
      }
    }
  ...
  test("should handle resource cleanup when closing sessions", async ({
    browser,
  }) => {
    ...
    expect(afterCleanup.pendingCalls).toBeLessThanOrEqual(
      initialResources.pendingCalls,
    );
    // Subscriptions might have some global ones, so just check they're reasonable
    expect(afterCleanup.subscriptions).toBeLessThan(
      afterCreation.subscriptions + 10,
    );
