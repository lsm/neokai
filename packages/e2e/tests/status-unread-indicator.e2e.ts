...
test.describe("Unread Message Indicator", () => {
  ...
  test("should show unread indicator when other session has new messages", async ({
    page,
  }) => {
  ...
  test("should clear unread indicator when session is clicked", async ({
    page,
  }) => {
    ...
    const staticBlueDot = sessionCard.locator(
      ".bg-blue-500:not(.animate-pulse)",
    );
