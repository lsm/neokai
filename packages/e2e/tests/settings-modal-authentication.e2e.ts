...
test.describe("Settings Modal - Authentication Status", () => {
  ...
  test.skip("should display Authentication Status section", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('h3:has-text("Authentication Status")'),
    ).toBeVisible();
  });

  test.skip("should show authenticated status with green indicator", async ({
    page,
  }) => {
    // TODO: Update test to match current SettingsModal structure
    await openSettingsModal(page);
    ...
    const authSection = page
      .locator('h3:has-text("Authentication Status")')
      .locator("..");
  ...
  test("should display auth method (API Key or OAuth)", async ({ page }) => {
    ...
    if (isAuthenticated) {
      ...
      const hasOAuthToken =
        (await page.locator("text=OAuth Token").count()) > 0;

      expect(hasApiKey || hasOAuth || hasOAuthToken).toBeTruthy();
    }
  });

  test.skip("should show environment variable setup instructions", async ({
    page,
  }) => {
    // TODO: Update test to match current SettingsModal structure
    await openSettingsModal(page);
