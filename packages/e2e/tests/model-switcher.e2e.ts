...
test.describe("Model Switcher UI", () => {
  ...
  test("should display model switcher in session status bar", async ({
    page,
  }) => {
  ...
  test("should show current model highlighted in dropdown", async ({
    page,
  }) => {
    await openModelSwitcher(page);
  ...
  test("should close dropdown when clicking the button again", async ({
    page,
  }) => {
    await openModelSwitcher(page);
