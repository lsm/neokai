...
test.describe("URL-based Session Routing", () => {
  ...
  test("should handle deep linking to a session", async () => {
    ...
    await expect(
      newPage.locator('textarea[placeholder*="Ask"]').first(),
    ).toBeVisible();
  ...
  test("should show home page when navigating to root URL", async () => {
    ...
    await expect(
      page
        .locator("text=Welcome to Liuboer")
        .or(page.locator('h2:has-text("Liuboer")')),
    ).toBeVisible();
  });
  ...
  test("should handle invalid session IDs gracefully", async () => {
    ...
    await page
      .waitForSelector('button:has-text("New Session")', { timeout: 10000 })
      .catch(() => {
