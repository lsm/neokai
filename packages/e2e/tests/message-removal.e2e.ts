...
test.describe("Message Output Removal", () => {
  ...
  test("should show tool output in message when present", async ({ page }) => {
    ...
    const _toolBlock = page
      .locator('[data-tool-use], .tool-use-block, [class*="tool"]')
      .first();
  ...
  test("should display message content after tool execution", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
  ...
  test("should maintain conversation after viewing tool output", async ({
    page,
  }) => {
    // Create a new session
    const newSessionButton = page.getByRole("button", {
