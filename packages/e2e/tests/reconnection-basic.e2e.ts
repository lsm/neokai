...
test.describe("Reconnection - Basic Message Sync", () => {
  ...
  test("should sync messages generated during disconnection", async ({
    page,
  }) => {
    ...
    await messageInput.fill(
      "Count from 1 to 5 with 1 second delay between each number",
    );
    ...
    const messagesBeforeDisconnect = await page
    ...
    const messagesAfterReconnect = await page
      .locator("[data-message-role]")
      .count();
