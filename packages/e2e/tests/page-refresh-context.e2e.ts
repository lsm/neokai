...
test.describe("Page Refresh - Context Persistence", () => {
  ...
  test("should restore context info percentage after refresh", async ({
    page,
  }) => {
  ...
  test("should persist token counts and metadata after refresh", async ({
    page,
  }) => {
    // Create session and send message
    const newSessionButton = page.getByRole("button", {
    ...
    // Wait for processing
    await waitForMessageProcessed(
      page,
      "Write a brief explanation of TypeScript.",
    );
  ...
  test("should restore context breakdown details after refresh", async ({
    page,
  }) => {
    // Create session and send message
    const newSessionButton = page.getByRole("button", {
    ...
    const contextIndicatorAfter = page
      .locator('span[class*="text-"][class*="-400"]')
      .first();
