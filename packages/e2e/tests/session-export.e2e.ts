...
test.describe("Session Export", () => {
  ...
  test("should show Export Chat option in session options menu", async ({
    page,
  }) => {
  ...
  test("should disable Export when disconnected", async ({ page }) => {
    ...
    const exportOption = page.locator(
      '[role="menuitem"]:has-text("Export Chat")',
    );
    ...
    const disabledOptionsButton = page
      .getByRole("button", { name: "Not connected" })
      .first();
