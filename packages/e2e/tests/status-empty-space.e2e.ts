...
test.describe("No Empty Space When No Indicator", () => {
  ...
  test("should not leave empty space when session is idle and read", async ({
    page,
  }) => {
    ...
    const titleRow = sessionCard
      .locator(".flex.items-center.gap-2.flex-1")
      .first();
    ...
    if (children.length > 0) {
      ...
      if (isIndicator) {
        ...
        if (
          hasContent === 0 &&
          hasClass?.includes("w-2.5") &&
          hasClass?.includes("h-2.5")
        ) {
          // This is the bug - empty spacer present
          throw new Error(
            "Empty spacer found when StatusIndicator should return null",
          );
        }
      }
    }
  ...
  test("title should be aligned consistently across sessions", async ({
    page,
  }) => {
