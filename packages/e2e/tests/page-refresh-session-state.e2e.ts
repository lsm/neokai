...
test.describe("Page Refresh - Session State Persistence", () => {
  ...
  test("should reset agent state to idle after refresh (expected behavior)", async ({
    page,
  }) => {
    // Create session
  ...
  test("should restore slash commands immediately after refresh", async ({
    page,
  }) => {
    // Create session
    const newSessionButton = page.getByRole("button", {
  ...
  test("should restore full session state including messages and title", async ({
    page,
  }) => {
    // Create session
    const newSessionButton = page.getByRole("button", {
    ...
    await expect(
      page.locator("text=What is React and why is it popular?"),
    ).toBeVisible();
