...
test.describe("Settings Modal - Settings Persistence", () => {
  ...
  test("should save model selection and show Saved indicator", async ({
    page,
  }) => {
...
test.describe("Settings Modal - MCP Servers", () => {
  ...
  test.skip("should display MCP Servers section in Global Settings", async ({
    page,
  }) => {
    // TODO: Update test to match current SettingsModal structure
    await openSettingsModal(page);
    ...
    await expect(
      page.locator("text=MCP servers from enabled setting sources"),
    ).toBeVisible();
  });
  ...
  test("should show MCP server configuration options", async ({ page }) => {
    ...
    if (hasServers) {
      ...
      const mcpSection = page
        .locator('label:has-text("MCP Servers")')
        .locator("..");
