...
test.describe("Chat Flow", () => {
  ...
  test("should create a new session and send a message", async ({ page }) => {
    ...
    await expect(
      page.locator("text=\"Hello, can you respond with just 'Hi!'?\""),
    ).toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(
      page.locator("text=/Hi|Hello|Greetings/i").first(),
    ).toBeVisible({
      timeout: 15000,
    });
  ...
  test("should show stop button while message is being processed", async ({
    page,
  }) => {
    // Create a new session
    await page
