...
test.describe('Session Operations', () => {
  test('should create a new session', async ({ app }) => {
    ...
    const sessionItems = app.locator('[data-testid="session-card"], .session-item, [role="listitem"]').filter({ hasText: /Session/ });
  ...
  test('should navigate to session and load messages', async ({ app }) => {
    ...
    await expect(app.getByRole('button', { name: /send/i }).or(app.locator('button[type="submit"]'))).toBeVisible();
  });
