...
test.describe('Multi-Tab Synchronization', () => {
	test('should sync session creation across tabs', async ({ browser }) => {
		...
		await expect(tabA.getByPlaceholder(/Ask, search, or make anything/i)).toBeVisible({
			timeout: 10000,
		});
	...
	test('should sync session deletion across tabs', async ({ browser }) => {
		...
		await expect(tabB.getByPlaceholder(/Ask, search, or make anything/i)).toBeVisible({
			timeout: 10000,
		});
		...
		await expect(tabB.getByRole('heading', { name: /Welcome to Liuboer/i })).toBeVisible({
			timeout: 10000,
		});
