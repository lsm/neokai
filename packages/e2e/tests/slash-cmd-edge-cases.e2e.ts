...
test.describe("Slash Command Autocomplete - Edge Cases", () => {
  ...
  test("should not show autocomplete for / in middle of text", async ({
    page,
  }) => {
  ...
  test("should show autocomplete for / with leading whitespace", async ({
    page,
  }) => {
  ...
  test("should handle command selection followed by more typing", async ({
    page,
  }) => {
    const textarea = getMessageInput(page);
