...
async function updateGlobalSettings(page: Page, updates: Record<string, unknown>): Promise<void> {
	const result = await page.evaluate(async (settingsUpdates) => {
		...
		return await hub.call('settings.global.update', { updates: settingsUpdates }, { timeout: 5000 });
	}, updates);
...
async function getGlobalSettings(page: Page): Promise<Record<string, unknown>> {
	return await page.evaluate(async () => {
		const hub = window.__messageHub || window.appState?.messageHub;
		if (!hub || !hub.call) throw new Error('MessageHub not available');
		return await hub.call('settings.global.get', {}, { timeout: 5000 }) as Record<string, unknown>;
	});
}
...
async function createSessionViaRPC(page: Page): Promise<string> {
	return await page.evaluate(async () => {
		const hub = window.__messageHub || window.appState?.messageHub;
		if (!hub || !hub.call) throw new Error('MessageHub not available');
		const result = await hub.call('session.create', { workspacePath: undefined }, { timeout: 15000 }) as {
			sessionId: string;
		};
...
async function getTestModels(page: Page): Promise<{ defaultModel?: string; testModel?: string }> {
	...
	const testModel = models.find(m => m.id.includes('opus'))
		?.id || models.find(m => m.id.includes('sonnet') && !m.id.includes('haiku'))
		?.id;
...
test.describe.serial('Global Settings Applied to New Sessions', () => {
	...
	test('first session uses current global settings for model, thinking, and autoScroll', async ({ page }) => {
	...
	test('after updating global settings, new session uses updated values for model, thinking, and autoScroll', async ({ page }) => {
		// Get available models to use for testing
		const { testModel } = await getTestModels(page);
		...
		const newModel = models.find(m =>
			m.id !== testModel && (m.id.includes('sonnet') || m.id.includes('opus'))
		)?.id || testModel;
	...
	test('new session uses default values when global settings are not explicitly set', async ({ page }) => {
