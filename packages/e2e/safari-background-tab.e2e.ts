...
test.describe("Safari Background Tab - E2E Tests", () => {
  test.describe("Session List Synchronization", () => {
    test("should show new sessions created while backgrounded", async ({
      page,
    }) => {
    ...
    test("should update session metadata changed while backgrounded", async ({
      page,
    }) => {
  ...
  test.describe("Message Synchronization", () => {
    test("should show messages generated while backgrounded", async ({
      page,
    }) => {
      ...
      const initialMsgCount = await page
        .locator('[data-testid="sdk-message"]')
        .count();
      ...
      const finalMsgCount = await page
        .locator('[data-testid="sdk-message"]')
        .count();
    ...
    test("should handle sending message after returning from background", async ({
      page,
    }) => {
      // 1. Create session
      await page.goto("http://localhost:9283");
  ...
  test.describe("Connection Status Indicators", () => {
    test("should show reconnecting state during validation", async ({
      page,
    }) => {
      await page.goto("http://localhost:9283");
      await page.waitForLoadState("networkidle");
      ...
      const statusPromise = page.evaluate(() => {
        ...
        return new Promise<string>((resolve) => {
          setTimeout(() => {
            resolve(
              (window as unknown as Record<string, unknown>).connectionState
                ?.value || "unknown",
            );
          }, 50);
        });
      ...
      const finalStatus = await page.evaluate(() => {
        return (window as unknown as Record<string, unknown>).connectionState
          ?.value;
      });
      expect(finalStatus).toBe("connected");
    });

    test("should maintain connected state if health check passes", async ({
      page,
    }) => {
      await page.goto("http://localhost:9283");
      await page.waitForLoadState("networkidle");
  ...
  test.describe("Multiple Background/Foreground Cycles", () => {
    test("should handle repeated background/foreground transitions", async ({
      page,
    }) => {
      await page.goto("http://localhost:9283");
      await page.waitForLoadState("networkidle");
