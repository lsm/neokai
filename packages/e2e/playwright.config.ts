...
const coverageOptions: CoverageReportOptions = {
  ...
  sourceFilter: (sourcePath: string) => {
    ...
    if (sourcePath.endsWith(".js") && !sourcePath.endsWith(".tsx.js"))
      return false;
...
export default defineConfig({
  ...
  reporter: collectCoverage
    ? [
        [
          "monocart-reporter",
          { name: "Liuboer E2E Coverage", coverage: coverageOptions },
        ],
        ["list"],
      ]
  ...
  webServer: {
    ...
    command:
      "cd ../web && bun run build && cd ../cli && NODE_ENV=test bun run main.ts",
