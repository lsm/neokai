...
export function useModelSwitcher(sessionId: string): UseModelSwitcherResult {
	...
	const loadModelInfo = useCallback(async () => {
		try {
			...
			// Fetch current model
			const { currentModel: modelId, modelInfo } = (await hub.query('session.model.get', {
				sessionId,
			})) as {
				currentModel: string;
			...
			const { models } = (await hub.query('models.list', {
				useCache: true,
			})) as {
				models: Array<{
					id: string;
	...
	const switchModel = useCallback(
		async (newModelId: string) => {
			...
			try {
				...
				const result = (await hub.query('session.model.switch', {
					sessionId,
					model: newModelId,
				})) as {
					success: boolean;
