...
describe("QuestionPrompt", () => {
  const mockOnResolved = vi.fn(
    (
      _state: "submitted" | "cancelled",
      _responses: QuestionDraftResponse[],
    ) => {},
  );
  ...
  describe("Multi Select Behavior", () => {
    it("should allow selecting multiple options", () => {
      const { container } = render(
        <QuestionPrompt
          sessionId="session-1"
          pendingQuestion={multiSelectQuestion}
        />,
      );

      const darkModeBtn = Array.from(container.querySelectorAll("button")).find(
        (btn) => btn.textContent?.includes("Dark Mode"),
      )!;
      const notificationsBtn = Array.from(
        container.querySelectorAll("button"),
  ...
  describe("Cancel Functionality", () => {
    it("should have skip button", () => {
      ...
      const skipButton = Array.from(container.querySelectorAll("button")).find(
        (btn) => btn.textContent?.includes("Skip Question"),
      );
  ...
  describe("Draft Loading", () => {
    it("should initialize from draft responses", () => {
      ...
      const { container } = render(
        <QuestionPrompt
          sessionId="session-1"
          pendingQuestion={questionWithDraft}
        />,
      );
  ...
  describe("Multiple Questions", () => {
    it("should render multiple questions", () => {
      ...
      const { container } = render(
        <QuestionPrompt
          sessionId="session-1"
          pendingQuestion={multiQuestionPrompt}
        />,
      );
