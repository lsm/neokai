    ...
    >
      ...
      <div class={cn("relative border-t", colors.border)}>
        <div
          ...
          style={
            !isExpanded && needsTruncation
              ? { maxHeight: `${PREVIEW_MAX_HEIGHT + 24}px` }
              : {}
          }
        >
          <pre
            ref={contentRef}
            class={cn("text-sm whitespace-pre-wrap font-mono", colors.text)}
          >
            {content}
          </pre>
        </div>
        ...
        {needsTruncation && (
          <div
            class={cn(
              "flex justify-center py-2 border-t bg-white dark:bg-gray-900",
              colors.border,
            )}
          >
            <button
