...
import {
  SlashCommandOutput,
  isHiddenCommandOutput,
} from "../SlashCommandOutput";
...
describe("SlashCommandOutput", () => {
  describe("Basic Rendering", () => {
    ...
    it('should show "Command Output" header', () => {
      const content =
        "<local-command-stdout>Test output</local-command-stdout>";
      const { container } = render(<SlashCommandOutput content={content} />);
  ...
  describe("Parsing", () => {
    it("should extract content from local-command-stdout tags", () => {
      const content =
        "<local-command-stdout>Extracted content here</local-command-stdout>";
      const { container } = render(<SlashCommandOutput content={content} />);
...
Line 3</local-command-stdout>`;
    ...
    it("should trim whitespace from extracted content", () => {
      const content =
        "<local-command-stdout>  Trimmed content  </local-command-stdout>";
      const { container } = render(<SlashCommandOutput content={content} />);
    ...
    it("should handle content with both stdout and surrounding text", () => {
      const content =
        "Before <local-command-stdout>Output</local-command-stdout> After";
      const { container } = render(<SlashCommandOutput content={content} />);
  ...
  describe("isHiddenCommandOutput Helper", () => {
    ...
    it("should return false for regular output", () => {
      const content =
        "<local-command-stdout>Regular output</local-command-stdout>";
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle output with nested angle brackets", () => {
      const content =
        "<local-command-stdout>Result: {count: 5}</local-command-stdout>";
      const { container } = render(<SlashCommandOutput content={content} />);
  ...
  describe("Command Types", () => {
    ...
    it("should render /context command output", () => {
      const content =
        "<local-command-stdout>Context: 50,000 tokens used</local-command-stdout>";
      const { container } = render(<SlashCommandOutput content={content} />);
    ...
    it("should render /cost command output", () => {
      const content =
        "<local-command-stdout>Total cost: $0.0125</local-command-stdout>";
      const { container } = render(<SlashCommandOutput content={content} />);
