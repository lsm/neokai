...
describe("ErrorOutput", () => {
  describe("hasErrorOutput", () => {
    ...
    it("should return false for content without stderr tags", () => {
      ...
      expect(
        hasErrorOutput("<local-command-stdout>OK</local-command-stdout>"),
      ).toBe(false);
    });
  ...
  describe("parseErrorOutput", () => {
    ...
    it("should return null for content without stderr tags", () => {
      ...
      expect(
        parseErrorOutput("<local-command-stdout>OK</local-command-stdout>"),
      ).toBe(null);
    });

    it("should trim whitespace from extracted content", () => {
      const content =
        "<local-command-stderr>  Error: 400  </local-command-stderr>";
