...
export function ToolResultCard({
  ...
  const shouldExpand =
    defaultExpanded !== undefined
      ? defaultExpanded
      : shouldExpandByDefault(toolName);
  const [isExpanded, setIsExpanded] = useState(shouldExpand);
  const [deleting, setDeleting] = useState(false);
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const customRenderer = hasCustomRenderer(toolName)
    ? getCustomRenderer(toolName)
    : null;
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to remove output";
  ...
  const getLineCountDisplay = () => {
    if (toolName === "Read") {
      ...
      if (content && typeof content === "string") {
        const lineCount = content.split("\n").length;
        return (
          <span class="text-xs text-green-700 dark:text-green-400 font-mono">
            +{lineCount}
          </span>
        );
      }
      ...
      if (oldText && newText) {
        const { addedLines, removedLines } = calculateDiffCounts(
          oldText,
          newText,
        );
        return (
  ...
  return (
    ...
    >
      ...
      {isExpanded && (
        <div
          class={cn(
            "p-3 border-t bg-white dark:bg-gray-900 space-y-3",
            colors.border,
          )}
        >
          ...
          toolName === "Read" &&
            ...
            (typeof output === "string" ||
              (typeof output === "object" &&
                "content" in output &&
                output.content &&
                typeof (output as Record<string, unknown>).content ===
                  "string")) ? (
            <CodeViewer
            ...
            <div>
              {variant === "detailed" && (
                <div class="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-2">
                  ...
                  {typeof input === "string" ? input.length : 0} characters)
                </div>
              )}
              <div
                class={cn(
              ...
              {variant === "detailed" && (
                <div class="text-xs text-gray-500 dark:text-gray-400 italic">
                  This is Claude's internal reasoning process before generating
                  the final response.
                </div>
              )}
            </div>
                  ...
                  <div class="flex items-center justify-between mb-2">
                    ...
                    {messageUuid && sessionId && !isOutputRemoved && (
                      ...
                      >
                        ...
                        <span>
                          {deleting ? "Removing..." : "Remove From Context"}
                        </span>
                      </button>
                    )}
                  </div>
                      ...
                      <div class="flex items-start gap-2">
                        ...
                        <div class="flex-1">
                          ...
                          <div class="text-xs text-amber-800 dark:text-amber-200">
                            This tool output has been removed from the Claude
                            Agent SDK session file to save context window space.
                            What you see here is stored in the database for
                            reference only and will not be sent to the agent in
                            future requests.
                          </div>
                        </div>
                      </div>
