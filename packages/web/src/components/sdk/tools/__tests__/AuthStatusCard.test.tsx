...
describe("AuthStatusCard", () => {
  ...
  describe("Output Messages", () => {
    it("should display output messages", () => {
      const { container } = render(
        <AuthStatusCard
          isAuthenticating={false}
          output={["Login successful", "Session created"]}
        />,
      );
    ...
    it("should not show output section when output is empty", () => {
      const { container } = render(
        <AuthStatusCard isAuthenticating={false} output={[]} />,
      );
      const outputDiv = container.querySelector(".whitespace-pre-wrap");
  ...
  describe("Error Display", () => {
    ...
    it("should show error in red color", () => {
      const { container } = render(
        <AuthStatusCard isAuthenticating={false} error="Failed" />,
      );
  ...
  describe("Compact Variant", () => {
    it("should render in compact style", () => {
      const { container } = render(
        <AuthStatusCard isAuthenticating={true} variant="compact" />,
      );
    ...
    it('should show "Authenticated" in compact mode when done', () => {
      const { container } = render(
        <AuthStatusCard isAuthenticating={false} variant="compact" />,
      );
  ...
  describe("Inline Variant", () => {
    it("should render as inline element", () => {
      const { container } = render(
        <AuthStatusCard isAuthenticating={false} variant="inline" />,
      );
    ...
    it("should show emoji for authenticating in inline mode", () => {
      const { container } = render(
        <AuthStatusCard isAuthenticating={true} variant="inline" />,
      );
  ...
  describe("Edge Cases", () => {
    it("should handle empty string error", () => {
      const { container } = render(
        <AuthStatusCard isAuthenticating={false} error="" />,
      );
    ...
    it("should handle many output messages", () => {
      const messages = Array(10)
        ...
        .map((_, i) => `Message ${i}`);
      const { container } = render(
        <AuthStatusCard isAuthenticating={false} output={messages} />,
      );
    ...
    it("should handle very long error message", () => {
      ...
      const { container } = render(
        <AuthStatusCard isAuthenticating={false} error={longError} />,
      );
