  ...
  switch (subagentType.toLowerCase()) {
    case "explore":
      return {
        ...
        badge:
          "bg-cyan-100 dark:bg-cyan-800/50 text-cyan-700 dark:text-cyan-300",
      ...
      >
        <div class="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span class={colors.icon}>
            {getSubagentIcon(input.subagent_type)}
          </span>
          ...
          {/* Description */}
          <span class={cn("text-sm font-medium truncate", colors.text)}>
            {input.description}
          </span>
        </div>
        ...
        {textBlocks.length > 0 && (
          <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-700 overflow-x-auto">
            {textBlocks.map((block, idx) => (
              <div
                key={idx}
                class="prose prose-sm dark:prose-invert max-w-full"
              >
