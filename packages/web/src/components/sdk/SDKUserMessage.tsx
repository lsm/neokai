...
export function SDKUserMessage({
  ...
  if (isReplay) {
    const content =
      typeof apiMessage.content === "string" ? apiMessage.content : "";
  ...
  const getSyntheticContentBlocks = ():
  ...
  const hasCommandOutput = (): boolean => {
    ...
    return /<local-command-stdout>[\s\S]*?<\/local-command-stdout>/.test(
      textContent,
    );
  };
            ...
            <div class="mt-3 space-y-2">
              {imageBlocks.map((img, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    class="rounded overflow-hidden border border-gray-600/50"
                  >
                    <img
