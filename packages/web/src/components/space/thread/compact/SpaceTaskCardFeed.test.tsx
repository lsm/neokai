...
describe('SpaceTaskCardFeed', () => {
	...
	it('filters out synthetic user messages (isSynthetic=true) — agent→agent handoffs', () => {
		...
		const rows = [syntheticRow, makeAssistantTextRow('r1', 'Coder Agent', 'working on it')];
		const { container } = render(
			<SpaceTaskCardFeed
				parsedRows={rows}
				taskId="task-1"
		...
		// Synthetic message must not appear — only the assistant row is visible.
		const rendered = container.querySelectorAll('[data-testid="sdk-message-renderer"]');
		expect(rendered.length).toBe(1);
		expect(rendered[0].textContent).toBe('working on it');
	});

	it('keeps real user messages (isSynthetic falsy) — human input from UI or RPC', () => {
		...
		const rows = [realUserRow, makeAssistantTextRow('r1', 'Task Agent', 'on it')];
		const { container } = render(
			<SpaceTaskCardFeed
				parsedRows={rows}
				taskId="task-1"
		...
		// Real user message must be visible alongside the assistant row.
		const rendered = container.querySelectorAll('[data-testid="sdk-message-renderer"]');
		expect(rendered.length).toBe(2);
