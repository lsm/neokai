...
export function GlobalToolsSettings() {
	...
	const updateMcpConfig = (key: 'allowProjectMcp' | 'defaultProjectMcp', value: boolean) => {
		...
		// If disabling permission, also disable default
		if (key === 'allowProjectMcp' && !value) {
			newConfig.mcp.defaultProjectMcp = false;
		}
		saveConfig(newConfig);
	};
					...
					<div class="space-y-3">
						...
						<div class="flex items-center justify-between p-3 rounded-lg bg-dark-700/50">
							<div class="flex items-center gap-3">
								...
								<div>
									<div class="text-sm text-gray-200">Project MCP (.mcp.json)</div>
									<div class="text-xs text-gray-500">Load MCP servers from workspace</div>
								</div>
							</div>
							<div class="flex items-center gap-4">
								{/* Allowed toggle */}
