// @ts-nocheck
/**
 * Tests for RoomSelfStatus Component
 */

import { render, cleanup } from '@testing-library/preact';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { RoomSelfStatus } from './RoomSelfStatus';
import type { RoomSelfState } from '@neokai/shared';

describe('RoomSelfStatus', () => {
	beforeEach(() => {
	...
	describe('Stopped State (no state)', () => {
		it('should render "Agent stopped" when state is null', () => {
			const { container } = render(<RoomSelfStatus roomId="room-1" state={null} />);
		...
		it('should show Start button when onAction is provided and state is null', () => {
			...
			const { container } = render(
				<RoomSelfStatus roomId="room-1" state={null} onAction={onAction} />
			);
	...
	describe('Lifecycle State Rendering', () => {
		const createMockState = (
			lifecycleState: RoomSelfState['lifecycleState'],
			overrides?: Partial<RoomSelfState>
		): RoomSelfState => ({
			roomId: 'room-1',
			lifecycleState,
		...
		it('should render idle state with gray badge', () => {
			...
			const { container } = render(<RoomSelfStatus roomId="room-1" state={state} />);
	...
	describe('Activity Description', () => {
		const createMockState = (overrides?: Partial<RoomSelfState>): RoomSelfState => ({
			roomId: 'room-1',
		...
		it('should show active sessions when no task or goal', () => {
			const state = createMockState({ activeWorkerSessionIds: ['pair-1', 'pair-2'] });
			const { container } = render(<RoomSelfStatus roomId="room-1" state={state} />);
			expect(container.textContent).toContain('2 active sessions');
		});

		it('should show singular session count', () => {
			const state = createMockState({ activeWorkerSessionIds: ['pair-1'] });
			const { container } = render(<RoomSelfStatus roomId="room-1" state={state} />);
	...
	describe('Action Buttons', () => {
		...
		it('should show Pause button when running (executing)', () => {
			...
			const { container } = render(
				<RoomSelfStatus roomId="room-1" state={state} onAction={onAction} />
			);
		...
		it('should show Start button for idle state', () => {
			const onAction = vi.fn();
	...
	describe('Relative Time Formatting', () => {
		const createMockState = (lastActivityAt: number): RoomSelfState => ({
			roomId: 'room-1',
			lifecycleState: 'idle',
			activeWorkerSessionIds: [],
			lastActivityAt,
