/**
 * RoomSettings - Unified room settings page
 *
 * Organized into logical sections:
 * 1. Basic Info (name, workspace paths)
 * 2. AI Configuration (models)
 * 3. Context (background, instructions)
 * 4. Execution (planning retries, review rounds, concurrency)
...
function SectionHeader({ children }: { children: preact.ComponentChildren }) {
	return (
		<h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">{children}</h3>
	);
}
	...
	return (
		<div class="flex flex-col h-full">
			...
			<div class="flex-1 overflow-y-auto p-4 space-y-8">
				...
				<section>
					<SectionHeader>{t('roomSettings.sectionAI')}</SectionHeader>
					<div class="space-y-5">
						{/* Default Model */}
						<div>
							...
							<p class="text-xs text-gray-500 mb-2">{t('roomSettings.defaultModelDesc')}</p>
							<select
								id="default-model"
						...
						<div>
							...
							<p class="text-xs text-gray-500 mb-2">{t('roomSettings.allowedModelsDesc')}</p>
				...
				{/* === Section 3: Execution === */}
				<section>
					<SectionHeader>{t('roomSettings.sectionExecution')}</SectionHeader>
					<div class="space-y-4">
			...
			{/* Sticky footer */}
			<div class="flex items-center justify-end gap-3 p-4 border-t border-dark-700 bg-dark-900">
				{isSaving.value && (
