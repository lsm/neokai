...
export function ToolsModal({ isOpen, onClose, session }: ToolsModalProps) {
  ...
  const settingSources = useSignal<SettingSource[]>([
    "user",
    "project",
    "local",
  ]);
  ...
  const isMcpAllowed = useComputed(
    () => globalConfig.value?.mcp?.allowProjectMcp ?? true,
  );
  ...
  const toggleServer = (serverName: string) => {
    if (disabledMcpServers.value.includes(serverName)) {
      ...
      disabledMcpServers.value = disabledMcpServers.value.filter(
        (s) => s !== serverName,
      );
    } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const hub = await connectionManager.getHub();
      const result = await hub.call<{ success: boolean; error?: string }>(
        "tools.save",
        {
      ...
      <div class="space-y-5">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mb-3">
            Configure the system prompt preset.
          </p>
          <div class="space-y-2">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mb-3">
            MCP servers from enabled setting sources. Enable servers you want to
            use in this session.
          </p>
          ...
          {!isMcpAllowed.value ? (
            ...
            <div class="space-y-3">
              ...
              {(["user", "project", "local"] as SettingSource[])
                ...
                .map((source) => {
                  const serversForSource =
                    mcpServersData.value?.servers[source] || [];
              ...
              {(["user", "project", "local"] as SettingSource[])
                ...
                .every(
                  (source) =>
                    (mcpServersData.value?.servers[source] || []).length === 0,
                ) && (
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mb-3">
            Enhanced tools provided by Liuboer.
          </p>
          <div class="space-y-2">
            {/* Memory Tool */}
            <label
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mb-3">
            Always available tools from Claude Agent SDK.
          </p>
        ...
        <div
          class={`pt-4 border-t ${borderColors.ui.secondary} flex gap-3 justify-end`}
        >
          <button
            type="button"
