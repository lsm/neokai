...
export function ErrorDialog({
  ...
  const categoryColor =
    ERROR_CATEGORY_COLORS[error.category as ErrorCategory] ||
    ERROR_CATEGORY_COLORS.system;
  const categoryIcon =
    ERROR_CATEGORY_ICONS[error.category as ErrorCategory] ||
    ERROR_CATEGORY_ICONS.system;
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Error Details" size="lg">
      <div class="space-y-4">
        ...
        {/* User Message */}
        <div
          class={`p-4 rounded-lg bg-dark-800 border ${borderColors.ui.default}`}
        >
                ...
                <dd class="text-gray-300 space-y-1">
                  <div>
                    ...
                    <span class="font-mono text-xs">
                      {error.sessionContext.sessionId}
                    </span>
                  </div>
...
function formatErrorReport(error: StructuredError): string {
  ...
  if (error.sessionContext) {
    ...
    if (error.sessionContext.processingState) {
      lines.push(
        `  Processing State: ${error.sessionContext.processingState.status}`,
      );
      ...
      if (error.sessionContext.processingState.messageId) {
        lines.push(
          `  Message ID: ${error.sessionContext.processingState.messageId}`,
        );
      }
    }
    lines.push("");
