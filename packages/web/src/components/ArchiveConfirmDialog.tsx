	...
	return (
		<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
			<div class={`bg-dark-800 border rounded-xl p-6 max-w-md mx-4 ${borderColors.ui.default}`}>
				<h3 class="text-lg font-semibold text-gray-100 mb-3">{t('archive.confirmTitle')}</h3>
				<p class="text-sm text-gray-300 mb-4">
					{t('archive.uncommittedChanges', { count: commitStatus.commits.length })}
				</p>
				<div
				...
				<div class="flex gap-3">
					...
					>
						{archiving ? t('archive.archiving') : t('archive.archiveAnyway')}
					</Button>
				</div>
			</div>
		</div>
