...
describe("ConnectionManager - Page Visibility Handling", () => {
    ...
    global.document.removeEventListener = vi.fn(
      (type: string, listener: EventListener) => {
        if (
          ...
          listener === visibilityChangeHandler
        ) {
  ...
  describe("Page Hidden Event", () => {
    it("should log when page becomes hidden", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Page hidden"),
      );
    });
  });
  ...
  describe("Page Visible Event - Reconnection Flow", () => {
    ...
    beforeEach(() => {
      ...
      (connectionManager as unknown as Record<string, unknown>).transport =
        mockTransport;
      (connectionManager as unknown as Record<string, unknown>).messageHub =
        mockMessageHub;
    });
    ...
    afterEach(() => {
      ...
      if (
        ...
        typeof (sessionStoreRefresh as { mockRestore: unknown }).mockRestore ===
          "function"
      ) {
      ...
      if (
        ...
        typeof (appStateRefresh as { mockRestore: unknown }).mockRestore ===
          "function"
      ) {
      ...
      if (
        ...
        typeof (globalStoreRefresh as { mockRestore: unknown }).mockRestore ===
          "function"
      ) {
    ...
    it("should call forceResubscribe when health check succeeds", async () => {
      const _appStateRefreshSpy = vi
        .spyOn(appState, "refreshAll")
    ...
    it("should refresh sessionStore, appState, and globalStore", async () => {
      const sessionStoreRefreshSpy = vi
        .spyOn(sessionStore, "refresh")
        .mockResolvedValue(undefined);
      const appStateRefreshSpy = vi
        .spyOn(appState, "refreshAll")
        .mockResolvedValue(undefined);
      const globalStoreRefreshSpy = vi
        .spyOn(globalStore, "refresh")
        .mockResolvedValue(undefined);
    ...
    it("should call refreshes in parallel (Promise.all)", async () => {
      ...
      const maxDiff =
        Math.max(...refreshStartTimes) - Math.min(...refreshStartTimes);
  ...
  describe("Error Handling", () => {
    ...
    beforeEach(() => {
      ...
      appStateRefreshSpy = vi
        .spyOn(appState, "refreshAll")
        .mockResolvedValue(undefined);
      globalStoreRefreshSpy = vi
        .spyOn(globalStore, "refresh")
        .mockResolvedValue(undefined);
  ...
  describe("No Connection Scenario", () => {
    ...
    it("should attempt reconnect when no connection exists", async () => {
      const reconnectSpy = vi
        .spyOn(connectionManager, "reconnect")
        .mockResolvedValue(undefined);
