...
class SessionStore {
  ...
  select(sessionId: string | null): Promise<void> {
    ...
    this.selectPromise = this.selectPromise.then(() =>
      this.doSelect(sessionId),
    );
        ...
        (delta) => {
          if (delta.added?.length) {
            ...
            const existingIds = new Set(
              this.sdkMessages.value.map((m) => m.uuid),
            );
            const newMessages = delta.added.filter(
    ...
    try {
      ...
      if (sessionState) {
        ...
        if (sessionState.commandsData?.availableCommands) {
          slashCommandsSignal.value =
            sessionState.commandsData.availableCommands;
        }
      }
      ...
      if (messagesState?.sdkMessages) {
        ...
        const snapshotTimestamp = (
          messagesState as unknown as { timestamp?: number }
        ).timestamp;
        ...
        if (snapshotTimestamp && currentMessages.length > 0) {
          ...
          const newerMessages = currentMessages.filter(
            (m) =>
              ((m as unknown as { timestamp?: number }).timestamp || 0) >=
              snapshotTimestamp,
          );
  ...
  async loadOlderMessages(
    ...
    try {
      const hub = await connectionManager.getHub();
      const result = await hub.call<{ sdkMessages: SDKMessage[] }>(
        "message.sdkMessages",
        {
          sessionId,
