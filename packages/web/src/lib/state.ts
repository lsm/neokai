      ...
      STATE_CHANNELS.SESSION_SDK_MESSAGES,
      {
        ...
        mergeDelta: (current, delta) => {
          ...
          return {
            ...
            sdkMessages: mergeSdkMessagesWithDedup(
              current.sdkMessages,
              typedDelta.added,
            ),
...
class ApplicationState {
  ...
  getSessionChannels(sessionId: string): SessionStateChannels {
    ...
    (async () => {
      if (previousChannels) {
        console.log(
          `[State] Switching session: cleaning up channels for ${previousSessionId}`,
        );
  ...
  private setupCurrentSessionAutoLoad(): void {
    ...
    const unsub = this.currentSessionIdSignal.subscribe(
        ...
        debounceTimer = setTimeout(() => {
          (async () => {
            ...
            if (previousSessionId && previousSessionId !== sessionId) {
              console.log(
                `[State] Cleaning up channels for previous session: ${previousSessionId}`,
              );
...
/** @public - Preact signal accessed via .value in components */
export const apiConnectionStatus = computed<
  import("@liuboer/shared").ApiConnectionState | null
>(() => {
  const system = systemState.value;
...
/** @public - Preact signal accessed via .value in components */
export const globalSettings = computed<
  import("@liuboer/shared").GlobalSettings | null
>(() => {
