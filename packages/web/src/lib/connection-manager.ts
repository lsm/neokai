...
if (typeof window !== "undefined") {
  ...
  (
    window as unknown as { slashCommandsSignal?: typeof slashCommandsSignal }
  ).slashCommandsSignal = slashCommandsSignal;
}
...
export class ConnectionManager {
  ...
  constructor(baseUrl?: string) {
    ...
    console.log(
      `[ConnectionManager] Initialized with baseUrl: ${this.baseUrl}`,
    );
  ...
  private waitForConnectionEventDriven(timeout: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        unsub();
        reject(
          new ConnectionTimeoutError(timeout, "WebSocket connection timeout"),
        );
      }, timeout);
  ...
  private notifyConnectionHandlers(): void {
    ...
    for (const handler of handlers) {
      try {
        ...
        console.error(
          "[ConnectionManager] Error in connection handler:",
          error,
        );
      }
    }
  ...
  private setupVisibilityHandlers(): void {
    ...
    this.visibilityHandler = () => {
      if (document.hidden) {
        console.log(
          "[ConnectionManager] Page hidden - connection may be paused by browser",
        );
      } else {
    ...
    this.pageHideHandler = () => {
      console.log(
        "[ConnectionManager] Page hiding - marking connection as stale",
      );
    };
  ...
  private async validateConnectionOnResume(): Promise<void> {
    if (!this.messageHub || !this.transport) {
      ...
      console.log(
        "[ConnectionManager] No existing connection on resume, initiating reconnect",
      );
    ...
    try {
      ...
      console.log(
        "[ConnectionManager] Subscriptions and state refreshed after validation",
      );
    } catch (error) {
      console.error(
        "[ConnectionManager] Connection validation failed, forcing reconnect:",
        error,
      );
  ...
  simulatePermanentDisconnect(): void {
    if (this.transport) {
      console.log(
        "[ConnectionManager] Simulating permanent disconnect for testing",
      );
      this.transport.close();
    }
    connectionState.value = "disconnected";
