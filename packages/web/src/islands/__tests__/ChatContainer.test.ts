...
describe("ChatContainer State Batching", () => {
  ...
  describe("requestAnimationFrame batching", () => {
    it("should defer state updates to requestAnimationFrame", () => {
      ...
      const mockSetSession = vi.fn((val: unknown) =>
        stateUpdates.push(`session:${val}`),
      );
      const mockSetContextUsage = vi.fn((val: unknown) =>
        stateUpdates.push(`context:${val}`),
      );
      const mockSetSending = vi.fn((val: unknown) =>
        stateUpdates.push(`sending:${val}`),
      );
      const mockSetCurrentAction = vi.fn((val: unknown) =>
        stateUpdates.push(`action:${val}`),
      );
      const mockSetStreamingPhase = vi.fn((val: unknown) =>
        stateUpdates.push(`phase:${val}`),
      );
  ...
  describe("agent status transitions", () => {
    ...
    it("should calculate streaming duration correctly", () => {
      ...
      const action =
        duration > 0 ? `Streaming (${duration}s)...` : "Streaming...";
