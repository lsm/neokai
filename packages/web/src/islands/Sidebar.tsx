...
export default function Sidebar() {
  ...
  const handleCreateSession = async () => {
    ...
    try {
      ...
      if (!response?.sessionId) {
        console.error(
          "[Sidebar] Invalid response from createSession:",
          response,
        );
      ...
      console.log(
        "[Sidebar] Session created successfully, sessionId:",
        response.sessionId,
      );
      ...
      setTimeout(() => {
        ...
        if (!currentSessionIds.includes(response.sessionId)) {
          console.warn(
            "[Sidebar] Session not in list after navigation - delta may be delayed",
          );
        } else {
  ...
  const handleSessionClick = (sessionId: string) => {
    ...
    console.log(
      "[Sidebar] Current signal value before update:",
      currentSessionIdSignal.value,
    );
    navigateToSession(sessionId);
    console.log(
      "[Sidebar] Current signal value after update:",
      currentSessionIdSignal.value,
    );
              ...
              <div class="flex items-center gap-2">
                ...
                {apiConnectionStatus.value?.status === "disconnected" && (
                  <>
                    ...
                    <span
                      class="text-red-300"
                      title={apiConnectionStatus.value.lastError}
                    >
                      Offline
                    </span>
                  </>
