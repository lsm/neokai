...
import type {
  SDKMessage,
  SDKSystemMessage,
} from "@liuboer/shared/sdk/sdk.d.ts";
...
export default function ChatContainer({ sessionId }: ChatContainerProps) {
  ...
  const resolvingQuestionsRef = useRef<Map<string, ResolvedQuestion>>(
    new Map(),
  );
  ...
  const [contextUsage, setContextUsage] = useState(
    sessionStore.contextInfo.value,
  );
  ...
  useEffect(() => {
    if (session?.metadata?.resolvedQuestions) {
      ...
      for (const [toolUseId, resolved] of Object.entries(
  ...
  const isProcessing =
    agentState.status === "processing" || agentState.status === "queued";
  const isCompacting =
  ...
  if (loading) {
    return (
      <div class="flex-1 flex flex-col bg-dark-900">
        <div
          class={`bg-dark-850/50 backdrop-blur-sm border-b ${borderColors.ui.default} p-4`}
        >
            ...
            <ContentContainer className="space-y-0 min-h-[calc(100%+1px)]">
              ...
              {messages.map((msg, idx) => (
                <SDKMessageRenderer
                  ...
                  sessionInfo={
                    msg.uuid
                      ? (maps.sessionInfoMap.get(msg.uuid) as
                          | SDKSystemMessage
                          | undefined)
                      : undefined
                  }
                  ...
                  onQuestionResolved={(state, responses) => {
                    ...
                    if (pendingQuestion) {
                      ...
                      resolvingQuestionsRef.current.set(
