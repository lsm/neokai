...
export function validateSystemPromptConfig(config: SystemPromptConfig): ValidationResult {
	...
	if (typeof config === 'object' && config !== null) {
		if (config.type !== 'preset') {
			return { valid: false, error: 'System prompt object must have type: "preset"' };
		}
		...
		if (config.append !== undefined) {
			...
			if (config.append.length > 50000) {
				return { valid: false, error: 'System prompt append too long (max 50KB)' };
			}
		}
		return { valid: true };
	}
...
export function validateToolsPresetConfig(config: ToolsPresetConfig): ValidationResult {
	...
	if (Array.isArray(config)) {
		for (const tool of config) {
			...
			if (tool.length > 200) {
				return { valid: false, error: `Tool name too long: ${tool.slice(0, 50)}...` };
			}
		}
		return { valid: true };
	}
	...
	if (typeof config === 'object' && config !== null) {
		...
		if (config.preset !== 'claude_code') {
			return { valid: false, error: 'Only "claude_code" tools preset is supported' };
		}
		return { valid: true };
	}
...
export function validateAgentDefinition(name: string, agent: AgentDefinition): ValidationResult {
	...
	if (name.length > 100) {
		return { valid: false, error: `Agent name too long: ${name.slice(0, 50)}...` };
	}
	...
	if (agent.description.length > 10000) {
		return { valid: false, error: `Agent ${name}: description too long (max 10KB)` };
	}
	...
	if (agent.prompt.length > 100000) {
		return { valid: false, error: `Agent ${name}: prompt too long (max 100KB)` };
	}
	...
	if (agent.disallowedTools !== undefined) {
		if (!Array.isArray(agent.disallowedTools)) {
			return { valid: false, error: `Agent ${name}: disallowedTools must be an array` };
		}
		for (const tool of agent.disallowedTools) {
			if (typeof tool !== 'string' || tool.length === 0) {
				return { valid: false, error: `Agent ${name}: invalid disallowed tool name` };
			}
		}
	}
	...
	if (config.network !== undefined) {
		...
		if (network.allowUnixSockets !== undefined) {
			if (!Array.isArray(network.allowUnixSockets)) {
				return { valid: false, error: 'network.allowUnixSockets must be an array' };
			}
		...
		if (network.allowedDomains !== undefined) {
			if (!Array.isArray(network.allowedDomains)) {
				return { valid: false, error: 'network.allowedDomains must be an array' };
			}
			for (const domain of network.allowedDomains) {
				if (typeof domain !== 'string' || domain.length === 0) {
					return { valid: false, error: 'Invalid domain in network.allowedDomains' };
				}
			}
		}
	...
	if (config.ignoreViolations !== undefined) {
		...
		if (ignore.file !== undefined) {
			if (!Array.isArray(ignore.file)) {
				return { valid: false, error: 'ignoreViolations.file must be an array' };
			}
			for (const pattern of ignore.file) {
				if (typeof pattern !== 'string' || pattern.length === 0) {
					return { valid: false, error: 'Invalid pattern in ignoreViolations.file' };
				}
			}
		}
		...
		if (ignore.network !== undefined) {
			if (!Array.isArray(ignore.network)) {
				return { valid: false, error: 'ignoreViolations.network must be an array' };
			}
			for (const pattern of ignore.network) {
				if (typeof pattern !== 'string' || pattern.length === 0) {
					return { valid: false, error: 'Invalid pattern in ignoreViolations.network' };
				}
			}
		}
	}
...
export function validateMcpServerConfig(name: string, config: McpServerConfig): ValidationResult {
	...
	if (name.length > 100) {
		return { valid: false, error: `MCP server name too long: ${name.slice(0, 50)}...` };
	}
	...
	if (!config.type || config.type === 'stdio') {
		...
		if (!stdioConfig.command || stdioConfig.command.length === 0) {
			return { valid: false, error: `MCP server ${name}: command is required for stdio type` };
		}

		if (stdioConfig.args !== undefined) {
			if (!Array.isArray(stdioConfig.args)) {
				return { valid: false, error: `MCP server ${name}: args must be an array` };
			}
			for (const arg of stdioConfig.args) {
				if (typeof arg !== 'string') {
					return { valid: false, error: `MCP server ${name}: all args must be strings` };
				}
			}
		}
		...
		if (stdioConfig.env !== undefined) {
			if (typeof stdioConfig.env !== 'object' || stdioConfig.env === null) {
				return { valid: false, error: `MCP server ${name}: env must be an object` };
			}
			for (const [key, value] of Object.entries(stdioConfig.env)) {
				if (typeof key !== 'string' || typeof value !== 'string') {
					return { valid: false, error: `MCP server ${name}: env values must be strings` };
				}
			}
		}
	...
	if (config.type === 'sse' || config.type === 'http') {
		...
		if (config.headers !== undefined) {
			if (typeof config.headers !== 'object' || config.headers === null) {
				return { valid: false, error: `MCP server ${name}: headers must be an object` };
			}
			for (const [key, value] of Object.entries(config.headers)) {
				if (typeof key !== 'string' || typeof value !== 'string') {
					return { valid: false, error: `MCP server ${name}: header values must be strings` };
				}
			}
		}
	...
	return { valid: false, error: `MCP server ${name}: invalid type '${config.type}'` };
}
...
export function validateOutputFormat(config: OutputFormatConfig): ValidationResult {
	...
	if (!config.schema || typeof config.schema !== 'object' || config.schema === null) {
		return { valid: false, error: 'Output format schema is required and must be an object' };
	}
	...
	try {
		...
		if (serialized.length > 100000) {
			return { valid: false, error: 'Output format schema too large (max 100KB)' };
		}
	} catch {
	...
	if (config.additionalDirectories !== undefined) {
		...
		for (const dir of config.additionalDirectories) {
			if (typeof dir !== 'string' || dir.length === 0) {
				return { valid: false, error: 'Invalid directory in additionalDirectories' };
			}
		}
	}
	...
	if (config.env !== undefined) {
		...
		for (const [key, value] of Object.entries(config.env)) {
			...
			if (value !== undefined && typeof value !== 'string') {
				return { valid: false, error: `env value for '${key}' must be a string or undefined` };
			}
		}
	}
