...
export class WebSocketServerTransport implements IMessageTransport {
    ...
    for (const clientId of staleClientIds) {
      ...
      if (ws) {
        try {
          ...
          this.logger.info(
            `Error closing stale connection ${clientId}:`,
            error,
          );
        }
      }
  ...
  registerClient(
    ...
    const connection: ClientConnection = {
      ...
      send: (data: string) => {
        ...
        if (queueSize >= this.maxQueueSize) {
          throw new Error(
            `Message queue full for client ${clientId} (max: ${this.maxQueueSize})`,
          );
        }
    ...
    this.logger.info(
      `Client registered: ${clientId} (session: ${connectionSessionId})`,
    );
  ...
  unregisterClient(clientId: string): void {
    ...
    for (const handler of this.clientDisconnectHandlers) {
      try {
        ...
        this.logger.error(
          `[${this.name}] Error in client disconnect handler:`,
          error,
        );
      }
    }
  ...
  handleClientMessage(message: HubMessage, clientId?: string): void {
    this.logger.info(
      `Received message: ${message.type} ${message.method}`,
      message,
    );
