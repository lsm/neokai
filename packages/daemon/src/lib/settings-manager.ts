...
export class SettingsManager {
  ...
  private readUserAttribution(): { commit?: string; pr?: string } | undefined {
    ...
    try {
      ...
      return userSettings.attribution as
  ...
  private async writeFileOnlySettings(settings: GlobalSettings): Promise<void> {
    ...
    if (settings.enableAllProjectMcpServers !== undefined) {
      localSettings.enableAllProjectMcpServers =
    ...
    if (settings.attribution !== undefined) {
      ...
      if (userAttribution !== undefined) {
        ...
        this.logger.log(
          "Using attribution from user settings:",
          userAttribution,
        );
      }
    }
  ...
  readFileOnlySettings(): Partial<GlobalSettings> {
    ...
    try {
      ...
      return {
        disabledMcpServers:
          (localSettings.disabledMcpjsonServers as string[]) || [],
        enabledMcpServers:
          (localSettings.enabledMcpjsonServers as string[]) || undefined,
        enableAllProjectMcpServers:
          (localSettings.enableAllProjectMcpServers as boolean) || undefined,
        askPermissions:
          ((localSettings.permissions as Record<string, unknown>)
            ?.ask as string[]) || undefined,
        excludedCommands:
          ((localSettings.sandbox as Record<string, unknown>)
            ?.excludedCommands as string[]) || undefined,
        outputStyle: (localSettings.outputStyle as string) || undefined,
        attribution:
          (localSettings.attribution as { commit?: string; pr?: string }) ||
          undefined,
      };
    } catch (error) {
  ...
  async toggleMcpServer(serverName: string, enabled: boolean): Promise<void> {
    this.logger.log(
      `Toggling MCP server ${serverName}: ${enabled ? "enabled" : "disabled"}`,
    );
  ...
  listMcpServersFromSources(): Record<SettingSource, McpServerInfo[]> {
    const globalSettings = this.getGlobalSettings();
    const enabledSources = globalSettings.settingSources || [
      "user",
      "project",
      "local",
    ];
   ...
   */
  getMcpServerSettings(): Record<
    string,
    { allowed?: boolean; defaultOn?: boolean }
  > {
    const globalSettings = this.getGlobalSettings();
