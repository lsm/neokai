...
export class ProviderService {
  ...
  getProviderEnvVars(session: Session): ProviderEnvVars {
    ...
    const apiKey =
      session.config.providerConfig?.apiKey || this.getProviderApiKey(provider);

    if (!apiKey) {
      console.warn(
        `[ProviderService] No API key found for provider ${provider}`,
      );
      return {};
    }
    ...
    const baseUrl =
      session.config.providerConfig?.baseUrl || definition.baseUrl;
  ...
  getTitleGenerationConfig(provider: Provider): {
    ...
    let modelId =
      definition.titleGenerationModel || definition.models[0] || "default";
    ...
    if (
      ...
      const model = models.find(
        (m: { family: string }) => m.family === modelId,
      );
      if (model) {
        modelId = model.id;
      }
  ...
  getEnvVarsForModel(modelId: string): ProviderEnvVars {
    ...
    if (!apiKey) {
      console.warn(
        `[ProviderService] No API key found for GLM model ${modelId}`,
      );
      return {};
    }
