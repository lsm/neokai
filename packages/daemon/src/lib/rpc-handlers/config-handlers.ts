...
export function setupConfigHandlers(
	...
	messageHub.handle('config.model.update', async (data) => {
		...
		if (settings.model) {
			...
			if (result.success) {
				...
				results.errors.push({ field: 'model', error: result.error || 'Failed to switch model' });
			}
		}
	...
	messageHub.handle('config.systemPrompt.update', async (data) => {
		...
		return { success: true, applied: false, message: 'Restart query to apply changes' };
	});
	...
	messageHub.handle('config.outputFormat.update', async (data) => {
		...
		// Persist
		await agentSession.updateConfig({ outputFormat: outputFormat || undefined });
	...
	messageHub.handle('config.updateBulk', async (data) => {
		...
		if (runtimeConfig.model) {
			...
			if (result.success) {
				...
				results.errors.push({ field: 'model', error: result.error || 'Failed' });
			}
		...
		if (runtimeConfig.maxThinkingTokens !== undefined) {
			...
			if (result.success) {
				...
				results.errors.push({ field: 'maxThinkingTokens', error: result.error || 'Failed' });
			}
		...
		if (runtimeConfig.permissionMode) {
			...
			if (result.success) {
				...
				results.errors.push({ field: 'permissionMode', error: result.error || 'Failed' });
			}
		...
		if (remainingKeys.length > 0) {
			...
			if (restartQuery) {
				...
				if (result.success) {
					...
					results.errors.push({ field: 'restart', error: result.error || 'Restart failed' });
