...
export function setupRewindHandlers(
	 ...
	 */
	messageHub.onQuery('rewind.checkpoints', async (data) => {
	 ...
	 */
	messageHub.onQuery('rewind.preview', async (data) => {
	 ...
	 */
	messageHub.onQuery('rewind.execute', async (data) => {
		const {
			sessionId,
			checkpointId,
			mode = 'files',
	 ...
	 */
	messageHub.onQuery('rewind.previewSelective', async (data) => {
	 ...
	 */
	messageHub.onQuery('rewind.executeSelective', async (data) => {
		const {
			sessionId,
			messageIds,
			mode = 'both',
