...
export function setupSessionHandlers(
  ...
  messageHub.handle("session.update", async (data, _ctx) => {
    const { sessionId: targetSessionId, ...updates } =
  ...
  messageHub.handle("session.archive", async (data, _ctx) => {
    ...
    const commitStatus = await worktreeManager.getCommitsAhead(
      session.worktree,
    );
  ...
  messageHub.handle("client.interrupt", async (data) => {
    ...
    daemonHub
      .emit("agent.interruptRequest", { sessionId: targetSessionId })
      .catch((err) => {
        console.error(
          "[client.interrupt] Error emitting interrupt event:",
          err,
        );
      });
  ...
  messageHub.handle("session.model.get", async (data) => {
    ...
    const { resolveModelAlias, getModelInfo } =
  ...
  messageHub.handle("worktree.cleanup", async (data) => {
    ...
    const cleanedPaths =
      await sessionManager.cleanupOrphanedWorktrees(workspacePath);
