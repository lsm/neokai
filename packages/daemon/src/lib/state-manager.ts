...
export class StateManager {
  ...
  private setupEventListeners(): void {
    ...
    this.eventBus.on("api.connection", (data) => {
      this.apiConnectionState =
        data as import("@liuboer/shared").ApiConnectionState;
      this.logger.log(
        "API connection state updated:",
        this.apiConnectionState.status,
      );
      this.broadcastSystemChange().catch((err: unknown) => {
        this.logger.error(
          "Failed to broadcast system state after API connection change:",
          err,
        );
      });
    });
   ...
   */
  private async broadcastSessionUpdateFromCache(
    sessionId: string,
  ): Promise<void> {
    try {
      ...
      if (!session) {
        this.logger.log(
          `Session ${sessionId} not in cache, skipped sessions delta broadcast`,
        );
        return;
      }
      ...
      this.logger.warn(
        `Failed to broadcast session update for ${sessionId}:`,
        error,
      );
    }
  }
  ...
  private async getSessionsState(): Promise<SessionsState> {
    ...
    const hasArchivedSessions = allSessions.some(
      (s) => s.status === "archived",
    );
  ...
  private async getSDKMessagesState(
    sessionId: string,
    since?: number,
  ): Promise<SDKMessagesState> {
    const agentSession = await this.sessionManager.getSessionAsync(sessionId);
    if (!agentSession) {
      throw new Error("Session not found");
    }
  ...
  async broadcastSessionsDelta(update: SessionsUpdate): Promise<void> {
    ...
    this.logger.debug(
      " Delta payload:",
      JSON.stringify({ ...update, version }, null, 2),
    );
    await this.messageHub.publish(
      channel,
      { ...update, version },
  ...
  async broadcastSessionStateChange(sessionId: string): Promise<void> {
    const version = this.incrementVersion(
      `${STATE_CHANNELS.SESSION}:${sessionId}`,
    );
