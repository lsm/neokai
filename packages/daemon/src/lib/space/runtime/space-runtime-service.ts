...
export class SpaceRuntimeService {
	...
	async attachSpaceToolsToMemberSession(
		...
		// Skip workflow node-agent sub-sessions (exec sessions `…:task:…:exec:…` and
		// eager-spawn sessions `…:task:…:agent:…`). Both are owned by TaskAgentManager,
		// which builds a SUB-SESSION-SPECIFIC `space-agent-tools` server via
		// `buildSpaceAgentToolsMcpServerForSubSession`. That server carries `myAgentName`
		// / `myNodeId` context required for gate writer authorization — context the generic
		// member-session server lacks. Merging the generic server here would silently
		// overwrite the specialised one (mergeRuntimeMcpServers overwrites on key
		// collision), breaking `write_gate` / `read_gate` / `approve_gate` for the
		// sub-session and disrupting the node-agent in-process MCP server connection.
		if (
