	...
	gates: [
		{
			...
			script: {
				...
				source: [
					...
					'if ! PR_JSON=$(gh pr view --json state,mergeable,mergeStateStatus --jq \'{state,mergeable,mergeStateStatus}\') || [ -z "$PR_JSON" ]; then',
					...
					'# UNKNOWN is conservative: gate retries until GitHub finishes computing',
					'if [ "$PR_MERGEABLE" != "MERGEABLE" ]; then',
					'  echo "PR is not mergeable (mergeable: ${PR_MERGEABLE:-unknown})" >&2',
					'  exit 1',
					'fi',
					'PR_STATUS=$(jq -r \'.mergeStateStatus\' <<< "$PR_JSON")',
					'# CLEAN, HAS_HOOKS, and UNKNOWN are all acceptable:',
					'# CLEAN = all checks passed, HAS_HOOKS = passed with merge hooks,',
					'# UNKNOWN = GitHub still computing (gate retries until resolved)',
					'if [ "$PR_STATUS" != "CLEAN" ] && [ "$PR_STATUS" != "HAS_HOOKS" ] && [ "$PR_STATUS" != "UNKNOWN" ]; then',
