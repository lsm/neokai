...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
	...
	"     gh api graphql -f query='query($owner:String!,$name:String!,$number:Int!){repository(owner:$owner,name:$name){pullRequest(number:$number){reviewThreads(first:100){nodes{id isResolved comments(first:1){nodes{url}}} pageInfo{hasNextPage}}}}}' -f owner=<owner> -f name=<repo> -F number=<number>",
	'   If any reviewThread has isResolved=false, do NOT merge. If the coder already',
	'   addressed it and replied, resolve it with the resolveReviewThread mutation;',
	'   otherwise request coder follow-up for the unresolved conversation URLs.',
	'3. Merge:',
