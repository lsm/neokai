...
export class SessionManager {
  ...
  constructor(
    ...
    this.sessionCache = new SessionCache(
      createAgentSession,
      (sessionId: string) => this.db.getSession(sessionId),
    );
  ...
  private setupEventSubscriptions(): void {
    ...
    const unsubMessageSendRequest = this.eventBus.on(
      "message.sendRequest",
      async (data) => {
        const { sessionId, messageId, content, images } = data;

        this.logger.info(
          `[SessionManager] Processing message:send:request for session ${sessionId}`,
        );
    ...
    const unsubMessagePersisted = this.eventBus.on(
      ...
      async (data) => {
        ...
        this.logger.info(
          `[SessionManager] Processing message:persisted for session ${sessionId}`,
        );
          ...
          if (needsWorkspaceInit) {
            ...
            const titleGenTask = this.sessionLifecycle
              ...
              .catch((error) => {
                ...
                this.logger.error(
                  `[SessionManager] Title generation failed:`,
                  error,
                );
              });
  ...
  async generateTitleAndRenameBranch(
    ...
    return this.sessionLifecycle.generateTitleAndRenameBranch(
      sessionId,
      userMessageText,
    );
  }
   ...
   */
  saveGlobalToolsConfig(
    config: ReturnType<typeof this.toolsConfigManager.getGlobal>,
  ) {
    ...
    try {
      ...
      for (const unsubscribe of this.eventBusUnsubscribers) {
        try {
          ...
          this.logger.error(
            `[SessionManager] Error during EventBus unsubscribe:`,
            error,
          );
        }
      }
      ...
      if (this.pendingBackgroundTasks.size > 0) {
        ...
        await Promise.all(Array.from(this.pendingBackgroundTasks)).catch(
          (error) => {
            this.logger.error(
              `[SessionManager] Error waiting for background tasks:`,
              error,
            );
          },
        );
      ...
      for (const [sessionId, agentSession] of this.sessionCache.entries()) {
        ...
        cleanupPromises.push(
          agentSession
            ...
            .catch((error) => {
              this.logger.error(
                `[SessionManager] Error cleaning up session ${sessionId}:`,
                error,
              );
            }),
        );
      ...
      this.logger.info(
        `[SessionManager] Total cleanup time: ${Date.now() - cleanupStart}ms`,
      );
    } catch (error) {
      // On failure, rollback to IDLE to allow retry
      this.cleanupState = CleanupState.IDLE;
      this.logger.error(
        `[SessionManager] Cleanup failed, state rolled back to IDLE:`,
        error,
      );
      throw error;
    }
  ...
  async cleanupOrphanedWorktrees(workspacePath?: string): Promise<string[]> {
    ...
    this.logger.info(
      `[SessionManager] Cleaning up orphaned worktrees in ${path}`,
    );
