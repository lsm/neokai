...
export class WorktreeManager {
  ...
  async createWorktree(
    ...
    try {
      ...
      if (existsSync(worktreePath)) {
        this.logger.warn(
          `[WorktreeManager] Worktree already exists: ${worktreePath}`,
        );
      ...
      this.logger.info(
        `[WorktreeManager] Successfully created worktree for session ${sessionId}`,
      );
  ...
  async removeWorktree(
    ...
    try {
      ...
      if (exists) {
        this.logger.info(
          `[WorktreeManager] Removing worktree: ${worktreePath}`,
        );
      ...
      if (deleteBranch && branch) {
        try {
          ...
          this.logger.warn(
            `[WorktreeManager] Failed to delete branch ${branch}:`,
            error,
          );
        }
      }
      ...
      this.logger.info(
        `[WorktreeManager] Successfully removed worktree: ${worktreePath}`,
      );
    } catch (error) {
    ...
    try {
      ...
      for (const worktree of worktrees) {
        ...
        if (
          ...
          this.logger.info(
            `[WorktreeManager] Removing orphaned worktree: ${worktree.path}`,
          );
          ...
          try {
            ...
            if (worktree.branch.startsWith("session/")) {
              try {
                ...
                this.logger.info(
                  `[WorktreeManager] Deleted orphaned branch: ${worktree.branch}`,
                );
              } catch (error) {
                this.logger.warn(
  ...
  async verifyWorktree(worktree: WorktreeMetadata): Promise<boolean> {
    ...
    if (!existsSync(worktreePath)) {
      this.logger.warn(
        `[WorktreeManager] Worktree directory missing: ${worktreePath}`,
      );
      return false;
    }
    ...
    if (!exists) {
      this.logger.warn(
        `[WorktreeManager] Worktree not in git worktree list: ${worktreePath}`,
      );
      return false;
    }
  ...
  async renameBranch(
    ...
    try {
      ...
      if (branchExists) {
        this.logger.warn(
          `[WorktreeManager] Branch ${newBranch} already exists, cannot rename`,
        );
        return false;
      }

      this.logger.info(
        `[WorktreeManager] Renaming branch ${oldBranch} to ${newBranch}`,
      );
      await git.branch(["-m", oldBranch, newBranch]);
      this.logger.info(
        `[WorktreeManager] Successfully renamed branch to ${newBranch}`,
      );
      return true;
    } catch (error) {
  ...
  private async getDefaultBranch(repoPath: string): Promise<string> {
    ...
    try {
      ...
      const defaultBranch = await git.raw([
        "symbolic-ref",
  ...
  async getCommitsAhead(
    ...
    try {
      ...
      try {
        ...
        this.logger.info(
          `[WorktreeManager] Base branch ${base} does not exist`,
        );
        return {
          hasCommitsAhead: false,
          commits: [],
