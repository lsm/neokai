...
export class ContextTracker {
    ...
    if (modelUsage) {
      ...
      if (modelName) {
        ...
        if (modelData?.contextWindow) {
          ...
          this.logger.log(
            `Updated context window size from SDK: ${this.contextWindowSize}`,
          );
        }
   ...
   */
  private async handleMessageDeltaUsage(usage: {
    output_tokens: number;
  }): Promise<void> {
  ...
  private async buildAndBroadcastContextInfo(
    ...
    const totalUsed =
      this.currentTurnInputTokens + this.currentTurnOutputTokens;
    ...
    if (
      source === "result" ||
      (source === "message_start" && this.currentTurnInputTokens > 0)
    ) {
      this.logger.log(
  ...
  private calculateBreakdown(
    ...
    const calcPercent = (tokens: number) =>
      (tokens / this.contextWindowSize) * 100;
