...
export class AgentSession {
  ...
  constructor(
    ...
    this.messageHandler.setCircuitBreakerTripCallback(
    ...
    if (session.availableCommands && session.availableCommands.length > 0) {
      ...
      this.logger.log(
        `Restored ${this.slashCommands.length} slash commands from session data`,
      );
    }
  ...
  private recoverOrphanedSentMessages(): void {
    try {
      ...
      const queuedMessages = this.db.getMessagesByStatus(
        this.session.id,
        "queued",
      );
      ...
      for (const msg of allMessages) {
        if (isSDKSystemMessage(msg) && msg.subtype === "init") {
          ...
          if (
            ...
            msgWithTimestamp.timestamp > latestInitTimestamp
          ) {
  ...
  private setupEventSubscriptions(): void {
    ...
    const unsubReset = this.daemonHub.on(
      ...
      async ({ sessionId, restartQuery }) => {
        this.logger.log(
          `Received agent.resetRequest (restartQuery: ${restartQuery})`,
        );
    ...
    const unsubMessagePersisted = this.daemonHub.on(
      ...
      async (data) => {
        this.logger.log(
          `Received message.persisted event (messageId: ${data.messageId})`,
        );
    ...
    this.logger.log(
      `DaemonHub subscriptions initialized with session filtering`,
    );
  }
  ...
  private async executeDeferredRestartIfPending(): Promise<void> {
    ...
    this.logger.log(
      `Agent became idle, executing deferred restart (reason: ${reason})`,
    );
  ...
  private async fetchAndCacheSlashCommands(): Promise<void> {
    if (
      !this.queryObject ||
      typeof this.queryObject.supportedCommands !== "function"
    ) {
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `Fetched ${this.slashCommands.length} slash commands from SDK`,
      );
  ...
  private async fetchAndCacheModels(): Promise<void> {
    ...
    try {
      ...
      const models = await getSupportedModelsFromQuery(
        this.queryObject,
        this.session.id,
      );
      ...
      if (models.length > 0) {
        this.logger.log(
          `Fetched ${models.length} models from SDK for session ${this.session.id}`,
        );
      } else {
  ...
  async startQueryAndEnqueue(
    ...
    this.messageQueue
      ...
      .catch(async (error) => {
        if (error instanceof Error && error.message === "Interrupted by user") {
          ...
          if (isTimeoutError) {
            this.logger.log(
              `Auto-recovering from timeout by resetting query...`,
            );
            try {
              ...
              this.messageQueue
                ...
                .catch(async (retryError) => {
                  this.logger.error(
                    `Retry of message ${messageId} also failed:`,
                    retryError,
                  );
              ...
              this.logger.error(
                `Auto-recovery from timeout failed:`,
                resetError,
              );
              await this.stateManager.setIdle();
            }
    ...
    this.daemonHub
      .emit("message.sent", { sessionId: this.session.id })
      .catch((err) => {
  ...
  private async ensureQueryStarted(): Promise<void> {
    ...
    if (this.interruptPromise) {
      this.logger.log(
        `Waiting for interrupt to complete before starting query...`,
      );
      try {
        await Promise.race([
  ...
  private async handleApiValidationError(error: unknown): Promise<boolean> {
    try {
      ...
      this.logger.log(
        `Handling API validation error as assistant message: ${statusCode}`,
      );
  ...
  private async runQuery(queryGeneration: number): Promise<void> {
    try {
      ...
      this.logger.log(
        `SDK cwd (session.workspacePath): ${this.session.workspacePath}`,
      );
      if (this.session.worktree) {
        this.logger.log(`Session uses worktree:`);
        this.logger.log(
          `  - Worktree path: ${this.session.worktree.worktreePath}`,
        );
      ...
      const optionsBuilder = new QueryOptionsBuilder(
        this.session,
      ...
      optionsBuilder.setCanUseTool(
        this.askUserQuestionHandler.createCanUseToolCallback(),
      );
      ...
      if (providerService.isGlmModel(modelId)) {
        this.logger.log(
          `Applied GLM env vars for model ${modelId} to process.env`,
        );
      }
      ...
      this.startupTimeoutTimer = setTimeout(() => {
        if (!this.firstMessageReceived) {
          ...
          this.logger.error(
            `SDK startup timeout: SDK did not respond within ${elapsed}ms. ` +
              ...
              (this.session.worktree
                ? ` (worktree: ${this.session.worktree.worktreePath})`
                : ""),
          );
          ...
          this.stateManager
            .setIdle()
            .catch((e) => this.logger.warn("Failed to set idle:", e));
        }
      }, STARTUP_TIMEOUT_MS);
      ...
      for await (const message of this.createAbortableQuery(
        ...
        if (this.startupTimeoutTimer && messageCount === 1) {
          ...
          this.logger.log(
            `SDK first message received after ${Date.now() - queryStartTime}ms`,
          );
        }
      ...
      if (!isStaleQuery) {
        ...
        if (Object.keys(this.originalEnvVars).length > 0) {
          ...
          this.logger.log(
            "Restored original environment variables after SDK query",
          );
        }
        ...
        this.logger.log(
          `Streaming query stopped (generation ${queryGeneration})`,
        );
      } else {
        this.logger.log(
  ...
  private async *createMessageGeneratorWrapper() {
    for await (const { message, onSent } of this.messageQueue.messageGenerator(
      this.session.id,
    )) {
      ...
      if (!isInternal) {
        await this.stateManager.setProcessing(
  ...
  private async *createAbortableQuery(
    ...
    try {
      ...
      if (signal.aborted) {
        this.logger.log(
          "Query already aborted at start of createAbortableQuery",
        );
        return;
      }
      ...
      while (!signal.aborted) {
        ...
        try {
          ...
          if (signal.aborted) {
            this.logger.log(
              "Query aborted via immediate signal check after race",
            );
            break;
          }
  ...
  async handleInterrupt(): Promise<void> {
    ...
    this.logger.log(
      `Handling interrupt (current state: ${currentState.status})...`,
    );
    ...
    try {
      ...
      if (
        this.queryObject &&
        typeof this.queryObject.interrupt === "function"
      ) {
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          this.logger.warn(
            `SDK interrupt() failed (may be expected):`,
            errorMessage,
          );
        }
      } else {
  ...
  async handleQuestionResponse(
    ...
    await this.askUserQuestionHandler.handleQuestionResponse(
      toolUseId,
      responses,
    );
  ...
  async setMaxThinkingTokens(
    ...
    try {
      ...
      if (!this.queryObject || !this.firstMessageReceived) {
        ...
        this.logger.log(
          `Max thinking tokens saved to config (query not active)`,
        );
        return { success: true };
      }
      ...
      if ("setMaxThinkingTokens" in this.queryObject) {
        await (
          this.queryObject as Query & {
            setMaxThinkingTokens: (t: number | null) => Promise<void>;
          }
  ...
  async setPermissionMode(
    ...
    try {
      // If query not running or transport not ready, just update config
      if (!this.queryObject || !this.firstMessageReceived) {
        this.session.config.permissionMode =
          mode as Session["config"]["permissionMode"];
        this.db.updateSession(this.session.id, { config: this.session.config });
  ...
  updateMetadata(updates: Partial<Session>): void {
    ...
    if (updates.workspacePath) {
      ...
      this.logger.log(
        `Updated SettingsManager workspacePath to: ${updates.workspacePath}`,
      );
    }
  ...
  async updateToolsConfig(
    ...
    try {
      ...
      if (tools?.disabledMcpServers !== undefined) {
        ...
        await this.settingsManager.setDisabledMcpServers(
          tools.disabledMcpServers,
        );
  ...
  async resetQuery(options?: {
    ...
    this.logger.log(
      `User-initiated query reset (restartQuery: ${restartQuery})...`,
    );
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(
        `[AgentSession] PHASE 3B: Waiting for SDK subprocess exit...`,
      );
    ...
    this.logger.log(
      `[AgentSession] Cleanup complete (${Date.now() - cleanupStart}ms total)`,
    );
  }
  ...
  async handleQueryTrigger(): Promise<{
    ...
    try {
      ...
      const savedMessages = this.db.getMessagesByStatus(
        this.session.id,
        "saved",
      );
  ...
  async sendQueuedMessagesOnTurnEnd(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Found ${queuedMessages.length} queued messages to send on turn end`,
      );
