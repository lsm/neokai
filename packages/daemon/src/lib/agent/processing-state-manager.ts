...
export class ProcessingStateManager {
  ...
  restoreFromDatabase(): void {
    ...
    if (!session?.processingState) {
      this.logger.log(
        "No persisted processing state found, starting with idle",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        this.logger.log(
          "Restored waiting_for_input state - user can still answer the question",
        );
      } else {
        this.processingState = restoredState;
        this.logger.log(
  ...
  private persistToDatabase(): void {
    try {
      ...
      this.logger.error(
        "Failed to persist processing state to database:",
        error,
      );
    }
  }
  ...
  async setWaitingForInput(
    ...
    this.logger.log(
      `Waiting for user input: ${pendingQuestion.questions.length} question(s)`,
    );
  }
  ...
  async updateQuestionDraft(
    draftResponses: PendingUserQuestion["draftResponses"],
  ): Promise<void> {
    if (this.processingState.status !== "waiting_for_input") {
  ...
  async updatePhase(phase: StreamingPhase): Promise<void> {
    if (this.processingState.status !== "processing") {
      this.logger.warn(
        `Cannot update phase to ${phase} - not in processing state`,
      );
      return;
    }
