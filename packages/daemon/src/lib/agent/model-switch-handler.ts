  ...
  async switchModel(newModel: string): Promise<ModelSwitchResult> {
    ...
    try {
      ...
      const currentProvider =
        providerService.detectProviderFromModel(currentResolvedModel);
      const newProvider =
        providerService.detectProviderFromModel(resolvedModel);
      const isCrossProviderSwitch = currentProvider !== newProvider;

      if (isCrossProviderSwitch) {
        logger.log(
          `Cross-provider switch detected: ${currentProvider} → ${newProvider}`,
        );
      }
      ...
      if (!queryObject || !transportReady) {
        ...
        logger.log(
          `Query restarted for cross-provider switch to ${resolvedModel}`,
        );
      } else {
