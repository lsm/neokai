 ...
 * ```
 */
export function createOutputLimiterHook(
  config: Partial<OutputLimiterConfig> = {},
): HookCallback {
  ...
  return async (input, _toolUseID, { signal: _signal }) => {
    ...
    const modifiedInput = limitToolInput(
      tool_name,
      tool_input,
      finalConfig,
      logger,
  ...
  switch (toolName) {
    case "Bash": {
      ...
      const simpleCommands =
        /^(pwd|cd|echo\s+"[^"]{0,50}"|ls(\s+-\w+)?(\s+\S+)?|which|whoami)$/;
      ...
      logger.log(
        `Bash: Added smart truncation (first ${headLines} + last ${tailLines} lines)`,
      );
...
export function getOutputLimiterConfigFromSettings(
  ...
  return {
    ...
    bash: {
      headLines:
        settingsLimiter?.bash?.headLines ?? DEFAULT_CONFIG.bash.headLines,
      tailLines:
        settingsLimiter?.bash?.tailLines ?? DEFAULT_CONFIG.bash.tailLines,
    },
    read: {
    ...
    grep: {
      maxMatches:
        settingsLimiter?.grep?.maxMatches ?? DEFAULT_CONFIG.grep.maxMatches,
    },
    glob: {
