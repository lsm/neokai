...
function runMigration5(db: BunDatabase): void {
  try {
    ...
    logger.log(
      "Running migration: Adding sdk_session_id column to sessions table",
    );
...
function runMigration6(db: BunDatabase): void {
  try {
    ...
    logger.log(
      "Running migration: Adding available_commands column to sessions table",
    );
...
function runMigration7(db: BunDatabase): void {
  try {
    ...
    logger.log(
      "Running migration: Adding processing_state column to sessions table",
    );
...
function runMigration8(db: BunDatabase): void {
  try {
    ...
    logger.log(
      "Running migration: Adding archived_at column to sessions table",
    );
...
function runMigration10(db: BunDatabase): void {
  try {
    ...
    logger.log(
      "Running migration: Adding send_status column to sdk_messages table",
    );
    db.exec(
...
function runMigration11(db: BunDatabase): void {
  try {
    ...
    logger.log(
      "Running migration: Adding sub-session columns to sessions table",
    );
    ...
    db.exec(
      `ALTER TABLE sessions ADD COLUMN sub_session_order INTEGER DEFAULT 0`,
    );
