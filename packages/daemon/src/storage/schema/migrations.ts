...
export function runMigrations(db: BunDatabase, createBackup: () => void): void {
	...
	// Migration 105: Automation Task System foundation. Creates the durable
...
/**
 * Migration 105 — Automation Task System foundation.
 *
 * Creates two new tables:
 *   - `automation_tasks`: long-lived user definitions for scheduled/continuous work
 *   - `automation_runs`: append-only ledger entries for each execution attempt
 *
 * This migration is additive and idempotent. It does not alter existing Room,
 * Space, Mission, or job queue tables.
 */
