...
describe("State Synchronization Integration", () => {
  ...
  describe("Global State Snapshot", () => {
    ...
    test("should include session data in snapshot", async () => {
      // Create a session
      const session1 = await callRPCHandler(ctx.messageHub, "session.create", { workspacePath: `${TMP_DIR}/test-workspace` }, {
  ...
  describe("Session State Snapshot", () => {
    test("should get session state snapshot", async () => {
      const created = await callRPCHandler(ctx.messageHub, "session.create", { workspacePath: `${TMP_DIR}/test-workspace` }, {
  ...
  describe("Per-Channel Versioning", () => {
    test("should increment version on each state change", async () => {
      // Get initial version
      const snapshot1 = await callRPCHandler(ctx.messageHub, 
        STATE_CHANNELS.GLOBAL_SNAPSHOT,
        {},
      );
      ...
      // Get new snapshot
      const snapshot2 = await callRPCHandler(ctx.messageHub, 
        STATE_CHANNELS.GLOBAL_SNAPSHOT,
        {},
      );
    ...
    test("should have independent versions for different channels", async () => {
      ...
      const sessionSnapshot = await callRPCHandler(ctx.messageHub, 
        STATE_CHANNELS.SESSION_SNAPSHOT,
        { sessionId: created.sessionId },
      );

      // Get global snapshot
      const globalSnapshot = await callRPCHandler(ctx.messageHub, 
        STATE_CHANNELS.GLOBAL_SNAPSHOT,
        {},
      );
