...
describe.skip('Agent State Event Broadcasting (DEPRECATED)', () => {
	...
	describe('State Transition: idle → queued → processing → idle', () => {
		test.skipIf(!hasAnyCredentials())(
			'should broadcast state events during message flow',
			async () => {
				// Create session
				const session = await callRPCHandler(ctx.messageHub, 'session.create', {
					workspacePath: `${TMP_DIR}/test-workspace`,
				...
				for (let i = 0; i < 20; i++) {
					try {
						...
						if (msg.type === MessageType.EVENT && msg.method === 'agent.state') {
							...
							if (status === 'queued') _gotQueued = true;
		...
		test.skipIf(!hasAnyCredentials())(
			'should include messageId in queued and processing states',
			async () => {
				const session = await callRPCHandler(ctx.messageHub, 'session.create', {
					workspacePath: `${TMP_DIR}/test-workspace`,
				});
				...
				console.log(
					'Received state events:',
					allStateEvents.map((e) => e.state.status)
				);
	...
	describe('State Transition: Interruption', () => {
		...
		test.skipIf(!hasAnyCredentials())(
			'should transition from processing to interrupted when message is interrupted',
			async () => {
				const session = await callRPCHandler(ctx.messageHub, 'session.create', {
					workspacePath: `${TMP_DIR}/test-workspace`,
				});
				...
				for (let i = 0; i < 10; i++) {
					...
					if (
						msg.type === MessageType.EVENT &&
						msg.method === 'agent.state' &&
						msg.data.state.status === 'processing'
					) {
						inProcessing = true;
						break;
					}
	...
	describe('State Channel Subscription', () => {
		test('should allow multiple clients to subscribe to agent.state', async () => {
			...
			await Promise.all([waitForWebSocketMessage(ws1), waitForWebSocketMessage(ws2)]);
	...
	describe('Concurrent Sessions', () => {
		test.skipIf(!hasAnyCredentials())(
			'should maintain independent state for multiple sessions',
			async () => {
				// Create two sessions
