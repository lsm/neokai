...
async function subscribeToEvent(ws: WebSocket, event: string, sessionId = "global"): Promise<string> {
  const messageId = generateUUID();
  ws.send(JSON.stringify({
    id: messageId,
    type: "SUBSCRIBE",
...
describe("MessageHub RPC Protocol", () => {
  ...
  test("should handle RPC error responses", async () => {
    ...
    const messageId = sendRPCCall(ws, 'session.get', { sessionId: 'non-existent-id' });
...
describe("MessageHub Pub/Sub Protocol", () => {
  ...
  test("should respect session-scoped event routing", async () => {
    ...
    // Create a session
    const createId = sendRPCCall(ws, 'session.create', {});
  ...
  test("should handle unsubscribe correctly", async () => {
    ...
    // Subscribe to event
    const subId = await subscribeToEvent(ws, testEvent, 'global');
