...
describe('Auto-Title Generation', () => {
	 ...
	 * Title generation now happens during workspace initialization (synchronous with first message)
	 * Just need to wait for agent to be idle - title should already be generated
	 */
	async function waitForTitleGeneration(
		agentSession: NonNullable<Awaited<ReturnType<typeof ctx.sessionManager.getSessionAsync>>>,
		timeoutMs = 20000
	): Promise<void> {
		// Wait for agent to be idle - workspace init completes during this time
		await waitForIdle(agentSession, timeoutMs);

		// Title should be generated by now (part of workspace initialization)
		const sessionData = agentSession.getSessionData();
		if (!sessionData.metadata.titleGenerated && sessionData.title === 'New Session') {
			console.warn('Title not generated after workspace initialization');
		}
	}
