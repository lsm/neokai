...
describe('Auto-Title Generation', () => {
	...
	/**
	 * Helper: Wait for title to be generated by the background queue
	 */
	async function waitForTitleGeneration(
		agentSession: NonNullable<Awaited<ReturnType<typeof ctx.sessionManager.getSessionAsync>>>,
		timeoutMs = 10000 // 10s for background queue processing
	): Promise<void> {
		const startTime = Date.now();
		while (Date.now() - startTime < timeoutMs) {
		...
		throw new Error(`Timeout waiting for title generation after ${timeoutMs}ms`);
	}
