...
describe('SDK Session File Manager', () => {
	describe('getSDKSessionFilePath', () => {
		it('should encode simple paths correctly', () => {
			...
			expect(result).toBe(
				join(homedir(), '.claude', 'projects', '-Users-alice-project', 'sdk-123.jsonl')
			);
		});
		...
		it('should handle multiple dots in path', () => {
			const result = getSDKSessionFilePath('/home/user/.config/.app/data', 'sdk-789');
			expect(result).toBe(
				join(homedir(), '.claude', 'projects', '-home-user--config--app-data', 'sdk-789.jsonl')
			);
		});
		...
		it('should handle paths without dots', () => {
			...
			expect(result).toBe(
				join(homedir(), '.claude', 'projects', '-var-lib-project', 'sdk-abc.jsonl')
			);
		});
		...
		it('should handle consecutive slashes', () => {
			...
			expect(result).toBe(
				join(homedir(), '.claude', 'projects', '-Users--double-slash', 'sdk-def.jsonl')
			);
		});
	});
});
