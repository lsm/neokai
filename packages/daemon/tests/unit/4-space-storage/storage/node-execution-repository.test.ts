...
describe('NodeExecutionRepository', () => {
	...
	describe('task_session_map maintenance with shared sessions', () => {
		// A long-lived agent session (e.g. a named coder) can be reused across
		// multiple node executions for the same task. The map is keyed
		// (task_id, session_id), so write-time bookkeeping must preserve the
		// row whenever ANY execution still references the session.

		const taskId = 'task-shared-1';
		const sharedSession = 'session-shared-coder';
		...
		function seedTask(): void {
			...
			(db as any)
				...
				.run(taskId, spaceId, 1, 'Shared session task', 'open', workflowRunId, now, now);
		}

		function mapRows(): Array<{
			taskId: string;
			sessionId: string;
			...
			return (db as any)
				.prepare(
					`SELECT task_id AS taskId, session_id AS sessionId, node_execution_id AS nodeExecutionId
					 FROM task_session_map WHERE kind = 'node_agent' AND task_id = ?`
				)
				.all(taskId) as Array<{
				taskId: string;
				sessionId: string;
		...
		it('delete preserves the map row when another execution still owns the session', () => {
			seedTask();
			const a = createExecution({
				workflowNodeId: 'node-A',
				agentName: 'coder',
				agentSessionId: sharedSession,
			});
			const b = createExecution({
				workflowNodeId: 'node-B',
				agentName: 'coder',
				agentSessionId: sharedSession,
			});

			// Map should hold exactly one row for this (task, session). The latest
			// writer (b) wins via INSERT OR REPLACE.
			expect(mapRows()).toHaveLength(1);
			expect(mapRows()[0].nodeExecutionId).toBe(b.id);

			// Deleting b — the current owner — must rebuild the row from a, not
			// drop it.
			repo.delete(b.id);

			const rowsAfter = mapRows();
			expect(rowsAfter).toHaveLength(1);
			expect(rowsAfter[0].sessionId).toBe(sharedSession);
			expect(rowsAfter[0].nodeExecutionId).toBe(a.id);
		});

		it('delete removes the map row only when no execution still references the session', () => {
			seedTask();
			const a = createExecution({
				workflowNodeId: 'node-A',
				agentName: 'coder',
		...
		it('updateSessionId rebuilds the map when another execution still owns the session', () => {
			...
			// b currently owns the row.
			expect(mapRows()[0].nodeExecutionId).toBe(b.id);

			// Rebinding b away from the shared session must NOT drop the map row
			// — a still binds it.
			repo.updateSessionId(b.id, 'session-new');
			const rows = mapRows();
			const sharedRow = rows.find((r) => r.sessionId === sharedSession);
			expect(sharedRow).toBeDefined();
			expect(sharedRow!.nodeExecutionId).toBe(a.id);
		});

		it('clearing agent_session_id removes the row only when no other execution binds it', () => {
			seedTask();
			const a = createExecution({
				workflowNodeId: 'node-A',
				agentName: 'coder',
			...
			repo.updateSessionId(a.id, null);
			expect(mapRows()).toHaveLength(0);
		});
	});
});
