...
describe('SpaceAgentRepository', () => {
	...
	describe('update', () => {
		...
		it('refreshes denormalised labels in task_session_map after a rename', () => {
			// Reviewer flagged: `task_session_map.label` is stamped at execution
			// write time and read directly by `spaceTaskMessages.byTask*`. Without
			// this refresh, task timelines would show outdated agent names
			// indefinitely until the next execution-level write touched the row.
			const agent = repo.create({ spaceId: 'space-1', name: 'Coder Old' });
			const now = Date.now();
			db.prepare(
				`INSERT INTO node_executions
				 (id, workflow_run_id, workflow_node_id, agent_name, agent_id,
				  agent_session_id, status, created_at, updated_at)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
			).run('ne-1', 'run-1', 'node-1', 'coder', agent.id, 'sess-1', 'in_progress', now, now);
			db.prepare(
				`INSERT INTO task_session_map
				 (task_id, session_id, kind, role, label, node_execution_id, created_at)
				 VALUES (?, ?, 'node_agent', 'coder', 'Coder Old', 'ne-1', ?)`
			).run('task-1', 'sess-1', now);

			repo.update(agent.id, { name: 'Coder New' });

			const row = db
				.prepare(`SELECT label FROM task_session_map WHERE task_id = ? AND session_id = ?`)
				.get('task-1', 'sess-1') as { label: string };
			expect(row.label).toBe('Coder New');
		});

		it('only refreshes labels for the agent that was renamed', () => {
			const renamed = repo.create({ spaceId: 'space-1', name: 'Renamed' });
			const sibling = repo.create({ spaceId: 'space-1', name: 'Sibling' });
			const now = Date.now();
			db.prepare(
				`INSERT INTO node_executions
				 (id, workflow_run_id, workflow_node_id, agent_name, agent_id,
				  agent_session_id, status, created_at, updated_at)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
			).run('ne-r', 'run-1', 'node-1', 'role-r', renamed.id, 'sess-r', 'in_progress', now, now);
			db.prepare(
				`INSERT INTO node_executions
				 (id, workflow_run_id, workflow_node_id, agent_name, agent_id,
				  agent_session_id, status, created_at, updated_at)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
			).run('ne-s', 'run-1', 'node-2', 'role-s', sibling.id, 'sess-s', 'in_progress', now, now);
			db.prepare(
				`INSERT INTO task_session_map
				 (task_id, session_id, kind, role, label, node_execution_id, created_at)
				 VALUES (?, ?, 'node_agent', 'role-r', 'Old R', 'ne-r', ?)`
			).run('task-1', 'sess-r', now);
			db.prepare(
				`INSERT INTO task_session_map
				 (task_id, session_id, kind, role, label, node_execution_id, created_at)
				 VALUES (?, ?, 'node_agent', 'role-s', 'Sibling', 'ne-s', ?)`
			).run('task-1', 'sess-s', now);

			repo.update(renamed.id, { name: 'New R' });

			const renamedLabel = (
				db.prepare(`SELECT label FROM task_session_map WHERE session_id = ?`).get('sess-r') as {
					label: string;
				}
			).label;
			const siblingLabel = (
				db.prepare(`SELECT label FROM task_session_map WHERE session_id = ?`).get('sess-s') as {
					label: string;
				}
			).label;
			expect(renamedLabel).toBe('New R');
			expect(siblingLabel).toBe('Sibling');
		});
	});
