...
describe('SpaceWorkflowManager', () => {
	...
	describe('eventInterests validation', () => {
		it('accepts valid event interests on create and persists them', () => {
						...
						agents: [
							{
								...
								name: 'coder',
								eventInterests: [
									{ topic: 'github/*/*/pull_request.*', label: 'PR events' },
									{ topic: 'github/neokai/web/pull_request.review_*' },
								],
							},
						],
					},
			...
			expect(wf.nodes[0].agents[0].eventInterests).toEqual([
				{ topic: 'github/*/*/pull_request.*', label: 'PR events' },
				{ topic: 'github/neokai/web/pull_request.review_*' },
			]);
		});

		it('accepts an omitted or empty eventInterests array', () => {
			expect(() =>
				manager.createWorkflow({
					spaceId: 'space-1',
					name: 'No Events Workflow',
					nodes: [
						{ id: 'node-1', name: 'Step', agents: [{ agentId: 'agent-1', name: 'coder' }] },
						{
							id: 'node-2',
							name: 'Empty Events',
							agents: [{ agentId: 'agent-1', name: 'coder', eventInterests: [] }],
						},
					],
					completionAutonomyLevel: 3,
				})
		...
		it('rejects non-array eventInterests', () => {
			expect(() =>
				manager.createWorkflow({
					spaceId: 'space-1',
					name: 'Bad Events Workflow',
					nodes: [
						{
							id: 'node-1',
							name: 'Step',
							agents: [
								{
									agentId: 'agent-1',
									name: 'coder',
									eventInterests: 'github/*/*/pull_request.*' as never,
								},
							],
						},
					],
			...
			).toThrow('node[0].agents[0].eventInterests must be an array');
		});

		it('rejects more than ten event interests per agent slot', () => {
			expect(() =>
				manager.createWorkflow({
					spaceId: 'space-1',
					name: 'Too Many Events Workflow',
					nodes: [
						{
							id: 'node-1',
							name: 'Step',
							agents: [
								{
									agentId: 'agent-1',
									name: 'coder',
									eventInterests: Array.from({ length: 11 }, () => ({
			...
			).toThrow('eventInterests must contain at most 10 entries');
		});

		it('rejects invalid event interest topic patterns with clear locations', () => {
			const invalidTopics = [
				['', 'Topic pattern must not be empty'],
				['github/*/pull_request.*', 'Topic pattern must have exactly 4 segments'],
				['github/*/*/pull_request', 'Topic pattern fourth segment must be resource.action'],
				['github/*/**/pull_request.*', 'Multi-segment "**" wildcard is not supported'],
				[
					'github/own*/*/pull_request.*',
					'Segment "own*" in position 2 uses an unsupported wildcard',
				],
				['github/*/*/pull request.*', 'Segment "pull request.*" contains invalid characters'],
			];

			for (const [topic, message] of invalidTopics) {
				expect(() =>
					manager.createWorkflow({
						spaceId: 'space-1',
				...
				).toThrow(`node[0].agents[0].eventInterests[0].topic is invalid: ${message}`);
			}
		});

		it('rejects non-string topics and labels', () => {
			expect(() =>
				manager.createWorkflow({
					spaceId: 'space-1',
					name: 'Bad Topic Type Workflow',
					nodes: [
						{
							id: 'node-1',
							name: 'Step',
							agents: [
								{
									agentId: 'agent-1',
									name: 'coder',
									eventInterests: [{ topic: 123 as never }],
								},
							],
						},
					],
			...
			).toThrow('node[0].agents[0].eventInterests[0].topic must be a non-empty string');

			expect(() =>
				manager.createWorkflow({
					spaceId: 'space-1',
					name: 'Bad Label Type Workflow',
					nodes: [
						{
							id: 'node-1',
							name: 'Step',
							agents: [
								{
									agentId: 'agent-1',
									name: 'coder',
									eventInterests: [{ topic: 'github/*/*/pull_request.*', label: 123 as never }],
								},
							],
						},
					],
			...
			).toThrow('node[0].agents[0].eventInterests[0].label must be a string');
		});

		it('validates eventInterests on update', () => {
			const created = manager.createWorkflow({
				spaceId: 'space-1',
				name: 'Update Events Workflow',
							...
							agents: [
								{
									agentId: 'agent-1',
									name: 'coder',
									eventInterests: [{ topic: 'github/*/*/pull_request' }],
								},
							],
						},
					],
				})
			).toThrow('node[0].agents[0].eventInterests[0].topic is invalid');
		});
	});
