...
describe('sdk-cli-resolver', () => {
	...
	describe('negative cache TTL', () => {
		...
		it('expires after 60s and retries resolution', () => {
			...
			// First call: sets negative cache
			const first = resolveSDKCliPath();
			expect(first).toBeUndefined();

			// Within TTL: still cached
			fakeTime = baseTime + 30_000;
			const within = resolveSDKCliPath();
			expect(within).toBeUndefined();

			// After TTL: retries (still fails with mocks)
			fakeTime = baseTime + 61_000;
			const after = resolveSDKCliPath();
			expect(after).toBeUndefined();

			Date.now = realNow;
		});

		it('clears negative cache on success after TTL expiry', () => {
			const realNow = Date.now;
			const baseTime = realNow();
			let fakeTime = baseTime;
			Date.now = () => fakeTime;
			...
			// After TTL: restore mocks so resolution succeeds from node_modules
			existsSyncSpy.mockRestore();
			execSyncSpy.mockRestore();
			execFileSyncSpy.mockRestore();
