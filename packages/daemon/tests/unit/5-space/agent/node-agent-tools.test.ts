...
describe('node-agent-tools: update_task', () => {
	let ctx: TestCtx;
	...
	test('delegates to onUpdateTask and returns the updated task', async () => {
				...
				content: [
					{
						...
						text: JSON.stringify({
							success: true,
							task: { id: args.task_id, title: args.title, description: args.description },
						}),
					},
				],
			}),
	...
	test('createNodeAgentMcpServer registers update_task only when callback is wired', () => {
		const withoutCallback = createNodeAgentMcpServer(makeConfig(ctx));
		const withoutRegistered = Object.keys(
			(withoutCallback as unknown as { instance: { _registeredTools: Record<string, unknown> } })
				.instance._registeredTools
		);
		expect(withoutRegistered).not.toContain('update_task');
		...
		expect(withRegistered).toContain('update_task');
	});
});
