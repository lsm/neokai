...
import { TopicTrie } from '../../../../src/lib/space/runtime/topic-trie';
...
describe('TopicTrie', () => {
	test('matches exact and segment-local wildcard topic patterns', () => {
		...
		expect(trie.lookup('github/lsm/neokai/pull_request.review_submitted').sort()).toEqual([
			'all-pr',
			'exact',
			'reviews',
		]);
		expect(trie.lookup('github/lsm/neokai/pull_request.comment_created')).toEqual(['all-pr']);
