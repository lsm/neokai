...
describe('SessionLifecycle', () => {
	...
	describe('update', () => {
		it('should update session in database', async () => {
			...
			expect(updateSessionSpy).toHaveBeenCalledWith('session-123', { title: 'Updated Title' });
		});
		...
		it('should update in-memory session when cached', async () => {
			...
			expect(mockAgentSession.updateMetadata).toHaveBeenCalledWith({ title: 'New Title' });
		});
	...
	describe('delete', () => {
		...
		it('should emit session.deleted via DaemonHub', async () => {
			...
			expect(emitSpy).toHaveBeenCalledWith('session.deleted', { sessionId: 'session-123' });
		});
