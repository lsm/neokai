...
describe('validateSystemPromptConfig', () => {
	...
	test('accepts valid claude_code preset', () => {
		const result = validateSystemPromptConfig({ type: 'preset', preset: 'claude_code' });
		expect(result.valid).toBe(true);
	});
	...
	test('rejects preset with invalid type', () => {
		const result = validateSystemPromptConfig({ type: 'invalid', preset: 'claude_code' } as never);
		expect(result.valid).toBe(false);
		expect(result.error).toContain('type: "preset"');
	});

	test('rejects preset with invalid preset name', () => {
		const result = validateSystemPromptConfig({ type: 'preset', preset: 'invalid' } as never);
		expect(result.valid).toBe(false);
...
describe('validateToolsPresetConfig', () => {
	...
	test('accepts valid claude_code preset', () => {
		const result = validateToolsPresetConfig({ type: 'preset', preset: 'claude_code' });
		expect(result.valid).toBe(true);
	});

	test('rejects preset with invalid type', () => {
		const result = validateToolsPresetConfig({ type: 'invalid', preset: 'claude_code' } as never);
		expect(result.valid).toBe(false);
		expect(result.error).toContain('type: "preset"');
	});

	test('rejects preset with invalid preset name', () => {
		const result = validateToolsPresetConfig({ type: 'preset', preset: 'custom' } as never);
		expect(result.valid).toBe(false);
		expect(result.error).toContain('claude_code');
	});
});
...
describe('validateMcpServerConfig', () => {
	...
	test('rejects stdio server without command', () => {
		const result = validateMcpServerConfig('memory', { type: 'stdio' } as never);
		expect(result.valid).toBe(false);
...
describe('validateOutputFormat', () => {
	...
	test('rejects non-object schema', () => {
		const result = validateOutputFormat({ type: 'json_schema', schema: 'not-an-object' as never });
		expect(result.valid).toBe(false);
	...
	test('rejects schema over 100KB', () => {
		...
		for (let i = 0; i < 5000; i++) {
			largeSchema[`property_${i}`] = { type: 'string', description: 'x'.repeat(20) };
		}
		const result = validateOutputFormat({ type: 'json_schema', schema: largeSchema });
		expect(result.valid).toBe(false);
...
describe('validateEnvConfig', () => {
	...
	test('rejects non-array additionalDirectories', () => {
		const result = validateEnvConfig({ additionalDirectories: '/path' as never });
		expect(result.valid).toBe(false);
	...
	test('rejects non-string in executableArgs', () => {
		const result = validateEnvConfig({ executableArgs: ['--watch', 123 as never] });
		expect(result.valid).toBe(false);
