		...
		test('should detect orphaned tool_result (missing tool_use)', () => {
			const messages = [
				...
				JSON.stringify({
					...
					message: {
						role: 'user',
						content: [{ type: 'tool_result', tool_use_id: 'orphan_tool_001', content: 'result' }],
					},
				}),
				// Assistant response
				JSON.stringify({
		...
		test('should detect multiple orphaned tool_results', () => {
			const messages = [
				...
				JSON.stringify({
					...
					message: {
						role: 'user',
						content: [{ type: 'tool_result', tool_use_id: 'orphan_001', content: 'result 1' }],
					},
				}),
				...
				JSON.stringify({
					...
					message: {
						role: 'user',
						content: [{ type: 'tool_result', tool_use_id: 'orphan_002', content: 'result 2' }],
					},
				}),
			];
		...
		test('should handle mixed valid and orphaned tool_results', () => {
			const messages = [
				...
				JSON.stringify({
					...
					message: {
						role: 'user',
						content: [{ type: 'tool_result', tool_use_id: 'valid_tool', content: 'valid' }],
					},
				}),
				...
				JSON.stringify({
					...
					message: {
						role: 'user',
						content: [{ type: 'tool_result', tool_use_id: 'orphan_tool', content: 'orphan' }],
					},
				}),
			];
		...
		test('should create backup before repair', () => {
			...
			const messages = [
				JSON.stringify({
					...
					message: { content: [{ type: 'tool_result', tool_use_id: 'orphan_t1' }] },
				}),
			];
			writeFileSync(testSessionFile, messages.join('\n') + '\n', 'utf-8');
			...
			const mockDb = createMockDb([
				{
					type: 'assistant',
					uuid: 'missing-parent',
					message: { content: [{ type: 'tool_use', id: 'orphan_t1', name: 'TaskOutput' }] },
				},
			]);
			...
			const mockDb = createMockDb([
				{
					...
					message: {
						role: 'assistant',
						content: [{ type: 'tool_use', id: 'orphan_t1', name: 'TaskOutput', input: {} }],
					},
				},
			]);
		...
		test('should report error when tool_use not found in DB', () => {
			...
			const messages = [
				JSON.stringify({
					...
					uuid: 'u1',
					parentUuid: 'missing-parent',
					message: { content: [{ type: 'tool_result', tool_use_id: 'unfindable_tool' }] },
				}),
			];
			writeFileSync(testSessionFile, messages.join('\n') + '\n', 'utf-8');
		...
		test('should return false when repair fails', () => {
			...
			const messages = [
				JSON.stringify({
					type: 'user',
					uuid: 'u1',
					message: { content: [{ type: 'tool_result', tool_use_id: 'unfindable' }] },
				}),
			];
			writeFileSync(testSessionFile, messages.join('\n') + '\n', 'utf-8');
