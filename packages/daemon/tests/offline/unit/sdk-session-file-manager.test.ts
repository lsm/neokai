...
describe("SDK Session File Manager", () => {
  ...
  describe("getSDKSessionFilePath", () => {
    test("should construct correct path with workspace encoding", () => {
      ...
      expect(path).toContain(
        ".claude/projects/-Users-test-project/session-123.jsonl",
      );
    });

    test("should handle dots and slashes in workspace path", () => {
      const path = getSDKSessionFilePath(
        "/Users/test/.hidden/project",
        "session-123",
      );
  ...
  describe("validateSDKSessionFile", () => {
    test("should return valid for non-existent file", () => {
      const result = validateSDKSessionFile(
        testWorkspacePath,
        "nonexistent-session",
      );
      ...
      const messages = [
        ...
        JSON.stringify({
          ...
          message: {
            role: "assistant",
            content: [
              { type: "tool_use", id: "tool_001", name: "Bash", input: {} },
            ],
          },
        }),
      ...
      const messages = [
        ...
        JSON.stringify({
          ...
          message: {
            role: "assistant",
            content: [
              { type: "tool_use", id: "valid_tool", name: "Bash", input: {} },
            ],
          },
        }),
        // Valid tool_result
  ...
  describe("repairSDKSessionFile", () => {
      ...
      const mockDb = createMockDb([
        {
          ...
          message: {
            content: [
              { type: "tool_use", id: "orphan_t1", name: "TaskOutput" },
            ],
          },
        },
      ]);
    ...
    test("should report error when tool_use not found in DB", () => {
      ...
      expect(result.errors[0]).toContain(
        "Could not find tool_use message for unfindable_tool",
      );
    });
  });
