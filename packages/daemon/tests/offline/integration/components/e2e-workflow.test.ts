...
describe("End-to-End Workflow Integration", () => {
  ...
  describe("Complete Session Lifecycle", () => {
    test("should handle full session lifecycle", async () => {
      ...
      // 4. Get SDK messages
      const messages = await callRPCHandler(
        ctx.messageHub,
        "message.sdkMessages",
        {
          sessionId: created.sessionId,
    ...
    test("should handle multiple sessions independently", async () => {
      ...
      const msgs1 = await callRPCHandler(
        ctx.messageHub,
        "message.sdkMessages",
        {
      ...
      const msgs2 = await callRPCHandler(
        ctx.messageHub,
        "message.sdkMessages",
        {
      ...
      const msgs3 = await callRPCHandler(
        ctx.messageHub,
        "message.sdkMessages",
        {
  ...
  describe("Concurrent Operations", () => {
    ...
    test("should handle concurrent session operations", async () => {
      ...
      const finalList = await callRPCHandler(
        ctx.messageHub,
        "session.list",
        {},
      );
  ...
  describe("State Snapshot Consistency", () => {
    test("should provide consistent global snapshot", async () => {
      ...
      // Get snapshot
      const snapshot = await callRPCHandler(
        ctx.messageHub,
