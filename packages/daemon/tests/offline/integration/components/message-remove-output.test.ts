...
describe("Message Remove Output Integration Tests", () => {
  ...
  describe("Remove Tool Result with SDK Session ID", () => {
    test("should remove tool_result content from message", async () => {
      ...
      const contentBefore = (
        targetMessageBefore!.message as Record<string, unknown>
      ).content as unknown[];
      ...
      const contentAfter = (
        targetMessageAfter!.message as Record<string, unknown>
      ).content as unknown[];
    ...
    test("should preserve other messages when removing tool_result", async () => {
      ...
      expect(JSON.stringify(firstMessageAfter)).toBe(
        JSON.stringify(firstMessageBefore),
      );
      expect(JSON.stringify(lastMessageAfter)).toBe(
        JSON.stringify(lastMessageBefore),
      );
    });
  });
  ...
  describe("Remove Tool Result with Fallback Search", () => {
    test("should find session file by Liuboer session ID when SDK session ID unavailable", async () => {
      ...
      const content = (targetMessage!.message as Record<string, unknown>)
        .content as unknown[];
  ...
  describe("Multiple Tool Results", () => {
    test("should only modify tool_result in specified message", async () => {
      ...
      const targetMessage = messagesAfter.find(
        (msg: unknown) =>
          (msg as Record<string, unknown>).uuid === targetMessageUuid,
      ) as Record<string, unknown>;
      const otherMessage = messagesAfter.find(
        (msg: unknown) =>
          (msg as Record<string, unknown>).uuid === otherMessageUuid,
      ) as Record<string, unknown>;

      // Target message should have placeholder
      const targetContent = (targetMessage.message as Record<string, unknown>)
        .content as unknown[];
      ...
      expect(
        (targetReplacedContent[0] as Record<string, unknown>).text,
      ).toContain("⚠️ Output removed by user");

      // Other message should be unchanged
      const otherContent = (otherMessage.message as Record<string, unknown>)
        .content as unknown[];
