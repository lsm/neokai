...
describe("SDK Context Validation Tests", () => {
  ...
  function createMockSDKSessionFile(
    ...
    const largeOutput = "This is a very large output. ".repeat(
      largeOutputSize / 30,
    );
  ...
  test("should significantly reduce session file size after removing large tool output", async () => {
    ...
    console.log("[Test] Size before removal:", {
      ...
      percentage:
        ((sizeBefore.toolResultSize / sizeBefore.totalSize) * 100).toFixed(1) +
        "%",
    });
    ...
    console.log("[Test] Size after removal:", {
      ...
      percentage:
        ((sizeAfter.toolResultSize / sizeAfter.totalSize) * 100).toFixed(1) +
        "%",
    });
  ...
  test("should replace large content with minimal placeholder", async () => {
    ...
    const placeholderText = (replacedContent[0] as Record<string, unknown>)
      .text as string;
  ...
  test("should preserve other messages and tool results unchanged", async () => {
          ...
          content: [
            {
              ...
              tool_use_id: "tool_001",
              content: [
                { type: "text", text: "Large output to remove".repeat(1000) },
              ],
            },
          ],
        },
          ...
          content: [
            {
              ...
              content: [
                { type: "text", text: "This should be preserved exactly" },
              ],
            },
          ],
        },
    ...
    expect(JSON.stringify(otherMessageAfter)).toBe(
      JSON.stringify(otherMessageBefore),
    );
