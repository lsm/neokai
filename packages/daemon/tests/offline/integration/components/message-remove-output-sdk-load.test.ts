...
describe("SDK Session Load Validation", () => {
  ...
  test("SDK should receive placeholder instead of large output when loading session", async () => {
    ...
    const toolResultMessageBefore = messagesBefore[2] as Record<
      string,
      unknown
    >;
    const contentBefore = (
      toolResultMessageBefore.content as unknown[]
    ...
    const contentAfter = (
      toolResultMessageAfter.content as unknown[]
    )[0] as Record<string, unknown>;
  ...
  test("SDK message array structure should remain valid after removal", async () => {
    ...
    console.log(
      "[Test] SDK would receive valid alternating user/assistant messages",
    );
    console.log(
      "[Test] Tool result structure preserved with placeholder content",
    );
  });
  ...
  test("Multiple removed outputs should all be placeholders when SDK loads", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const resultContent = (content.content as unknown[])[0] as Record<
        string,
        unknown
      >;
    ...
    console.log(
      "[Test] All removed outputs are placeholders when SDK loads session",
    );
  });
});
