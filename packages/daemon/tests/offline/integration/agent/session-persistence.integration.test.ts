...
describe("Session Persistence Integration", () => {
  ...
  describe("Commands Broadcasting", () => {
    it("should persist commands and emit update event", async () => {
      ...
      expect(
        typeof commandsEvent.data === "object" && commandsEvent.data !== null,
      ).toBe(true);
      if (
        typeof commandsEvent.data === "object" &&
        commandsEvent.data !== null
      ) {
        expect("commands" in commandsEvent.data).toBe(true);
        expect((commandsEvent.data as { commands: string[] }).commands).toEqual(
          commands,
        );
      }
    });
  ...
  describe("Page Refresh Simulation", () => {
    it("should restore persisted state after session reload", async () => {
      ...
      expect(reloadedSession!.availableCommands).toEqual([
        "clear",
        "help",
        "context",
      ]);
      ...
      const newStateManager = new ProcessingStateManager(
        sessionId,
        env.daemonHub,
        env.db,
      );
