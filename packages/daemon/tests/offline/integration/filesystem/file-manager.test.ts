...
describe("FileManager", () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(
      join(testWorkspace, "subdir", "nested.txt"),
      "Nested content",
    );
  ...
  describe("path traversal protection", () => {
    ...
    it("should allow subdirectory access", async () => {
      await expect(fileManager.pathExists("subdir/nested.txt")).resolves.toBe(
        true,
      );
    });
  });
  ...
  describe("readFile", () => {
    ...
    it("should throw error when trying to read directory", async () => {
      await expect(fileManager.readFile("subdir")).rejects.toThrow(
        "Path is a directory",
      );
    });
  ...
  describe("listDirectory", () => {
    ...
    it("should list recursively when recursive=true", async () => {
      ...
      expect(files.some((f) => f.path.includes("subdir/nested.txt"))).toBe(
        true,
      );
  ...
  describe("getFileTree", () => {
    ...
    it("should throw error for non-existent path", async () => {
      await expect(fileManager.getFileTree("nonexistent")).rejects.toThrow(
        "Path not found",
      );
    });
