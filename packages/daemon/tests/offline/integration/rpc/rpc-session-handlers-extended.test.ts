...
describe('Session RPC Handlers - Extended', () => {
	...
	describe('session.model.get', () => {
		test('should get current model info', async () => {
			...
			const response = await sendRpcCall(ws, 'session.model.get', { sessionId });
	...
	describe('worktree.cleanup', () => {
		test('should return success with empty cleanedPaths', async () => {
			...
			expect(response.type).toBe('RESULT');
			const data = response.data as { success: boolean; cleanedPaths: string[]; message: string };
			expect(data.success).toBe(true);
	...
	describe('models.list', () => {
		test('should list available models', async () => {
			...
			expect(response.type).toBe('RESULT');
			const data = response.data as { models: Array<{ id: string; display_name: string }> };
		...
		test('should support forceRefresh parameter', async () => {
			...
			const response = await sendRpcCall(ws, 'models.list', { forceRefresh: true });
		...
		test('should support useCache parameter', async () => {
			...
			const response = await sendRpcCall(ws, 'models.list', { useCache: false });
