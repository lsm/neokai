...
describe("WorktreeManager - Path Generation", () => {
  ...
  beforeEach(async () => {
    ...
    testRepoPath1 = path.join(
      TMP_DIR,
      `test-git-repo-1-${timestamp}-${random}`,
    );
    testRepoPath2 = path.join(
      TMP_DIR,
      `test-git-repo-2-${timestamp}-${random}`,
    );
    ...
    for (const worktreePath of createdWorktrees) {
      try {
        if (fs.existsSync(worktreePath)) {
          ...
          const repoPath = worktreePath.startsWith(
            path.join(homedir(), ".liuboer", "projects"),
          )
            ? testRepoPath1
