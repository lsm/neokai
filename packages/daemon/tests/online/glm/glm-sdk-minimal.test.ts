...
describe("GLM SDK - Minimal Direct Test", () => {
  test("should work with GLM via haiku model (glm-4.5-air)", async () => {
    ...
    console.log(
      "[GLM Test] Starting minimal SDK test with haiku → glm-4.5-air...",
    );
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          () =>
            reject(new Error("SDK timeout - no messages received after 20s")),
          20000,
        );
      });
  ...
  test("should work with GLM via default/sonnet model (glm-4.7)", async () => {
    ...
    try {
      ...
      expect(responseText.length).toBeGreaterThan(0);
      console.log(
        "[GLM Test] SUCCESS - GLM works with default (sonnet) model!",
      );
    } finally {
      // Restore env vars
      if (originalAuthToken !== undefined) {
