...
} from "../../offline/integration-test-utils";
...
describe("GLM Provider Integration", () => {
  ...
  describe("ProviderService - Model-based Detection", () => {
    ...
    it("should detect provider from model ID", () => {
      ...
      expect(providerService.detectProviderFromModel("default")).toBe(
        "anthropic",
      );
    ...
    it("should return correct env vars for GLM model ID", () => {
      ...
      try {
        ...
        expect(envVars.ANTHROPIC_BASE_URL).toBe(
    ...
    it("should return empty env vars for Anthropic model IDs", () => {
      ...
      expect(
        Object.keys(providerService.getEnvVarsForModel("default")).length,
      ).toBe(0);
      expect(
        Object.keys(providerService.getEnvVarsForModel("opus")).length,
      ).toBe(0);
      expect(
        Object.keys(providerService.getEnvVarsForModel("haiku")).length,
      ).toBe(0);
    });
    ...
    it("should get default model for each provider", () => {
      ...
      expect(providerService.getDefaultModelForProvider("anthropic")).toBe(
        "default",
      );
    ...
    it("should validate provider switch correctly", () => {
      ...
      const anthropicResult =
        providerService.validateProviderSwitch("anthropic");
      ...
      const glmWithKey = providerService.validateProviderSwitch(
        "glm",
        "some-api-key",
      );
  ...
  describe("QueryOptionsBuilder - Model-based Env Var Injection", () => {
    ...
    it("should not inject env vars for opus/haiku models", async () => {
      ...
      const haikuBuilder = new QueryOptionsBuilder(
        haikuSession,
        settingsManager,
      );
