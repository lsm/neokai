...
describe('MCP Tools Toggle Integration', () => {
	...
	test(
		...
		async () => {
			...
			const session1 = (await daemon.messageHub.query('session.get', {
				sessionId,
			})) as { session: Session };
			...
			const mcpServers = (await daemon.messageHub.query('settings.mcp.listFromSources', {
				sessionId,
			})) as {
			...
			const disableResult = (await daemon.messageHub.query('tools.save', {
				sessionId,
				tools: {
			...
			const enableResult = (await daemon.messageHub.query('tools.save', {
				sessionId,
				tools: {
					useClaudeCodePreset: true,
					settingSources: ['user', 'project', 'local'],
	...
	test(
		...
		async () => {
			...
			await daemon.messageHub.query('tools.save', {
				sessionId,
				tools: {
					useClaudeCodePreset: true,
					settingSources: ['user', 'project', 'local'],
			...
			const session = (await daemon.messageHub.query('session.get', {
				sessionId,
			})) as { session: Session };
