	...
	describe('SIGINT during active SDK query', () => {
		test(
			...
			async () => {
				...
				const sessionResult = (await daemon.messageHub.query('session.create', {
				...
				await daemon.messageHub.query('message.send', {
					sessionId,
				...
				const sessionResult2 = (await daemon.messageHub.query('session.get', {
					sessionId,
