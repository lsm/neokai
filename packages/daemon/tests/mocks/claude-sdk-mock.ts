...
export class MockQuery implements AsyncIterable<MockSDKMessage> {
  ...
  constructor(
    ...
    if (
      ...
      Symbol.asyncIterator in options.prompt
    ) {
  ...
  async *[Symbol.asyncIterator](): AsyncGenerator<MockSDKMessage> {
    while (!this.aborted) {
      ...
      const result = await new Promise<IteratorResult<MockSDKMessage>>(
        (resolve) => {
 ...
 */
export async function installClaudeSDKMock(): Promise<
  ReturnType<typeof createClaudeSDKMock>
> {
